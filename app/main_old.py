...
async def health_check():
    ...
    try:
        ...
        return {
            ...
            "prediction_accuracy": "Training mode: "
            + str(ml_status.get("is_trained", False)),
            "sample_data": {
...
async def get_goal_betting_data():
    ...
    goal_stats = {
        "total_goals": {
            "matches": len(matches),
            "average_per_match": sum(m.get("total_goals", 0) for m in matches)
            / len(matches)
            if matches
            else 0,
            "over_2_5_percentage": len(
                [m for m in matches if m.get("total_goals", 0) > 2.5]
            )
            / len(matches)
            * 100
            if matches
        ...
        "high_scoring_teams": [
            team["name"]
            for team in real_epl_service.get_epl_teams()[:5]
...
async def get_enhanced_team_stats(team_name: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting enhanced stats: {str(e)}"
        )
...
async def generate_real_time_prediction(home_team: str, away_team: str):
    ...
    try:
        ...
        prediction_service = PredictionService(
            db=None
        )  # We'll handle the None case in the service

        prediction = prediction_service.generate_real_time_prediction(
            home_team, away_team
        )
    ...
    try:
        ...
        return {
            ...
            "insights": {
                ...
                "team_forms": {"home_form": home_form, "away_form": away_form},
            },
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
...
async def predict_match_historical(home_team: str, away_team: str):
    ...
    try:
        prediction = await historical_ml_predictor.predict_match_comprehensive(
            home_team, away_team
        )
...
async def get_ml_status():
    ...
    try:
        ...
        return {
            "advanced_ml": {
                "status": "trained"
                if advanced_status.get("is_trained")
                else "untrained",
            ...
            "historical_ml": {
                "status": "trained"
                if historical_status.get("is_trained")
                else "untrained",
            ...
            "recommendation": "Use historical ML for maximum accuracy"
            if historical_status.get("is_trained")
            else "Train historical models first",
            "season": "2025-26",
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
