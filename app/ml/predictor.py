...
class MatchPredictor:
    ...
    def _get_team_stats(
        self, team_id: int, db: Session, last_n_matches: int = 10
    ) -> Dict[str, float]:
    ...
    def _get_head_to_head_stats(
        self, home_team_id: int, away_team_id: int, db: Session, last_n_matches: int = 5
    ) -> Dict[str, float]:
    ...
    def _predict_heuristic(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        home_goals_avg = next(
            (v for k, v in feature_dict.items() if "goals_for" in k and "home" in k),
            1.5,
        )
        away_goals_avg = next(
            (v for k, v in feature_dict.items() if "goals_for" in k and "away" in k),
            1.5,
        )
        ...
        first_half_pred = (
            total_goals_pred * 0.45
        )  # Typically ~45% of goals in first half
