...
from ..services.enhanced_epl_service import enhanced_epl_service  # legacy v1 for fallback
try:
...
try:
    from ..services.team_seed_metrics import team_seed_metrics_service, FEATURE_VERSION as TEAM_FEATURE_VERSION
except Exception:  # pragma: no cover
...
class AdvancedMLPredictor:
    ...
    def __init__(self):
        ...
        self.feature_columns = [
            ...
            'goal_difference_gap', 'position_difference',
            'home_win_rate', 'away_win_rate',
            'home_form_score', 'away_form_score',
            'home_recent_goals', 'away_recent_goals',
            'h2h_home_advantage', 'h2h_avg_goals', 'h2h_over_2_5_rate',
            'home_advantage_multiplier', 'is_top_6_clash', 'is_relegation_battle',
            'strength_disparity',
            # v2 engineered features (added for diversity)
            'attack_matchup_index', 'defense_matchup_index', 'form_gap',
            'tempo_gap', 'creation_gap', 'finishing_gap', 'pair_variance'
        ]
        # Market augmentation feature columns (appended if historic odds available)
        self.market_feature_columns = [
            'market_home_implied','market_draw_implied','market_away_implied',
            'market_overround','market_fav_prob','market_prob_skew'
        ]
    ...
    def generate_training_data(self, num_samples: int = 1000) -> pd.DataFrame:
        ...
        try:
            ...
            if not teams or len(teams) < 4:
                logger.warning("Insufficient real team stats; using synthetic training dataset")
                return self._create_synthetic_training_data(num_samples)
            ...
            engineered_cols = ['attack_matchup_index','defense_matchup_index','form_gap','tempo_gap','creation_gap','finishing_gap','pair_variance']
            for col in engineered_cols:
                if col not in df.columns:
                    # Deterministic-ish filler based on hash of index to avoid constant zeros
                    df[col] = [random.uniform(-0.5,0.5) if 'gap' in col or col=='pair_variance' else random.uniform(0.4,2.5) for _ in range(len(df))]
            ...
            if 'total_corners' not in df.columns:
                ...
                df['total_corners'] = (base_tc + np.random.normal(0,2,len(df))).clip(lower=3)
    ...
    def _augment_with_market_rows(self, df):
        ...
        logger.info(f"Augmented with {len(market_rows)} market odds rows; new total samples {len(combined)}")
        logger.info(f"Feature columns now {len(self.feature_columns)} (including market features)")
        return combined
    ...
    def _simulate_match_outcome(self, features: Dict) -> Dict:
        ...
        home_strength = features.get('home_goals_per_game', 1.5) * features.get('home_advantage_multiplier', 1.1)
        away_strength = features.get('away_goals_per_game', 1.5)
        
        # Defensive adjustments
        home_attack_vs_away_defense = home_strength / max(features.get('away_goals_conceded_per_game', 1.5), 0.5)
        away_attack_vs_home_defense = away_strength / max(features.get('home_goals_conceded_per_game', 1.5), 0.5)
        
        # Apply form factors
        home_expected = home_attack_vs_away_defense * features.get('home_form_score', 0.5) * 1.5
        away_expected = away_attack_vs_home_defense * features.get('away_form_score', 0.5) * 1.5
        ...
        first_half_goals = np.random.binomial(total_goals, 0.45)  # ~45% of goals in first half
    ...
    def _add_realistic_noise(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        noise_columns = ['home_goals_per_game', 'away_goals_per_game', 'home_form_score', 'away_form_score']
    ...
    def train_models(self, training_data: Optional[pd.DataFrame] = None):
        ...
        if not any(c.startswith('market_') for c in training_data.columns):
            ...
            if mrows:
                ...
                logger.info(f"Train-time market merge applied: appended {len(mrows)} market rows; new size={len(training_data)}")
    ...
    def _ensure_market_features(self, df: pd.DataFrame):
        ...
        for c in market_cols:
            if c not in self.feature_columns:
                ...
                added += 1
        market_rows = int((~df['market_home_implied'].isna()).sum()) if 'market_home_implied' in df.columns else 0
        logger.info(f"Market feature integration: detected {len(market_cols)} market columns, added {added}, rows with market data={market_rows}")
        ...
        if 'total_goals' in data.columns:
            ...
            if mask_total.any():
                ...
                for model_name, model in self.models['goals_total'].items():
                    ...
                    self.training_accuracy[f'total_goals_{model_name}'] = 1.0 / (1.0 + mse)
                    if hasattr(model, 'feature_importances_'):
                        self.feature_importance[f'total_goals_{model_name}'] = model.feature_importances_.tolist()
        # First half goals
        if 'first_half_goals' in data.columns:
            ...
            if mask_fh.any():
                ...
                for model_name, model in self.models['goals_first_half'].items():
                    ...
                    self.training_accuracy[f'first_half_goals_{model_name}'] = 1.0 / (1.0 + mse)
        ...
        if 'match_result' in data.columns:
            ...
            if mask_res.any():
                ...
                for model_name, model in self.models['match_result'].items():
                    ...
                    if hasattr(model, 'feature_importances_'):
                        self.feature_importance[f'match_result_{model_name}'] = model.feature_importances_.tolist()
    ...
    def _train_corners_models(self, X: np.ndarray, data: pd.DataFrame):
        ...
        if 'total_corners' not in data.columns:
            logger.warning("Skipping corner model training: total_corners column missing")
            return
        mask_c = ~data['total_corners'].isna()
        if not mask_c.any():
            logger.warning("Skipping corner model training: no non-null total_corners values")
            return
        try:
            X_c = X[mask_c.values]
            ...
            for model_name, model in self.models['corners_total'].items():
                ...
                self.training_accuracy[f'total_corners_{model_name}'] = 1.0 / (1.0 + mse)
        except Exception as e:
            logger.warning(f"Corner model training failed: {e}")

    def predict_match(self, home_team: str, away_team: str, league: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                norm_home, norm_away = _normalize_team(home_team), _normalize_team(away_team)
            ...
            try:
                ...
                if fv and isinstance(fv, str) and TEAM_FEATURE_VERSION not in fv:
                    adj_seed = hashlib.sha256(f"fv_adj_{home_team}_{away_team}_{fv}".encode()).hexdigest()
                    r = random.Random(int(adj_seed[:12], 16))
                    scale = 1 + r.uniform(-0.035, 0.035)
                    for k, v in list(features.items()):
                        if isinstance(v, (int, float)) and k not in ('position_difference', 'goal_difference_gap'):
            ...
            for i, (model_name, model) in enumerate([item for item in self.models['goals_total'].items() if item[0] != 'ensemble_weights']):
            ...
            try:
                ...
                xg_total_goals = float(eg.get('lambda_home', 1.3) + eg.get('lambda_away', 1.2))
            except Exception:
                eg = None
            ...
            cfg = _load_blend_config() or {}
            try:
                w_xg = float(cfg.get('goals_xg_weight', os.getenv('ML_GOALS_XG_WEIGHT', '0.5')))
            except Exception:
                w_xg = 0.5
            ...
            adjusted_total = max(0.0, base_total - float(total_bias)) if total_bias else base_total
            ...
            predictions['total_goals_confidence'] = self._calculate_confidence('goals_total')
            
            # First half goals prediction
            fh_goals_preds = []
            weights = self.models['goals_first_half']['ensemble_weights']
            
            for i, (model_name, model) in enumerate([item for item in self.models['goals_first_half'].items()
                                                   if item[0] != 'ensemble_weights']):
                pred = model.predict(X_scaled)[0]
                fh_goals_preds.append(pred * weights[i])
            
            predictions['first_half_goals'] = max(0, sum(fh_goals_preds))
            predictions['first_half_goals_confidence'] = self._calculate_confidence('goals_first_half')
            ...
            for i, (model_name, model) in enumerate([item for item in self.models['match_result'].items() if item[0] != 'ensemble_weights']):
            ...
            try:
                if eg is None:
                    eg = expected_goals_service.get_expected_goals(home_team, away_team)
                dist = expected_goals_service.scoreline_distribution(float(eg.get('lambda_home', 1.3)), float(eg.get('lambda_away', 1.2)))
                xg_probs = np.array([float(dist.get('draw_prob', 0.3)), float(dist.get('home_win_prob', 0.35)), float(dist.get('away_win_prob', 0.35))])
            except Exception:
                xg_probs = np.array([1/3, 1/3, 1/3], dtype=float)
            # Blend ML vs xG (env tunable)
            cfg = cfg or _load_blend_config() or {}
            try:
                w_xg_probs = float(cfg.get('result_xg_weight', os.getenv('ML_RESULT_XG_WEIGHT', '0.4')))
            except Exception:
                w_xg_probs = 0.4
            ...
            try:
                ...
                calibrated = calibration_service.apply_if_ready({'H': float(smoothed[1]), 'D': float(smoothed[0]), 'A': float(smoothed[2])}, league=league)
                final_probs = np.array([calibrated['D'], calibrated['H'], calibrated['A']])
            except Exception:
            ...
            predictions['match_result_confidence'] = max(0.0, min(1.0, 0.5 * float(np.max(final_probs)) + 0.5 * margin))
            
            # Corners prediction
            corners_preds = []
            weights = self.models['corners_total']['ensemble_weights']
            
            for i, (model_name, model) in enumerate([item for item in self.models['corners_total'].items()
                                                   if item[0] != 'ensemble_weights']):
                pred = model.predict(X_scaled)[0]
            ...
            try:
                if (not total_corners) or total_corners < 1.0:
                    ...
                    activity = 1.0 + max(0.0, abs(pace)) * 0.15 + max(0.0, abs(creation_gap)) * 0.10
                    # Base correlates with expected total goals; clamp sensible bounds
                    base_goals = float(predictions.get('total_goals', adjusted_total)) if 'predictions' in locals() else adjusted_total
            ...
            predictions['total_corners_confidence'] = self._calculate_confidence('corners_total')
            ...
            try:
                ...
                if eg is None:
                    eg = expected_goals_service.get_expected_goals(home_team, away_team)
                dist = expected_goals_service.scoreline_distribution(float(eg.get('lambda_home', 1.3)), float(eg.get('lambda_away', 1.2)))
                predictions['over_2_5_goals_probability'] = float(dist.get('over_2_5_prob', self._calculate_over_probability(predictions['total_goals'])))
            except Exception:
                predictions['over_2_5_goals_probability'] = self._calculate_over_probability(predictions['total_goals'])
            # BTTS from Poisson (1 - P(H=0) - P(A=0) + P(H=0,A=0))
            try:
                if eg is None:
                    eg = expected_goals_service.get_expected_goals(home_team, away_team)
                lam_h = float(eg.get('lambda_home', 1.3)); lam_a = float(eg.get('lambda_away', 1.2))
                p_h0 = np.exp(-lam_h); p_a0 = np.exp(-lam_a); p_both0 = np.exp(-(lam_h + lam_a))
                predictions['both_teams_score_probability'] = float(max(0.0, min(1.0, 1.0 - p_h0 - p_a0 + p_both0)))
            except Exception:
                predictions['both_teams_score_probability'] = self._calculate_btts_probability(features)
            predictions['clean_sheet_probability'] = 1.0 - predictions['both_teams_score_probability']
            ...
            predictions['match_context'] = {
                ...
                'feature_version': features.get('feature_version') or getattr(self, 'feature_version', 'fv1')
            }
            ...
            logger.error(f"Error in ML prediction for {original_home} vs {original_away}: {e}")
        ...
        try:
            ...
            if _v2_feature_service is not None:
                ...
                if feat_data:
                    ...
                    features = enhanced_epl_service.get_match_prediction_features(home_team, away_team).get('features', {})
            else:
                features = enhanced_epl_service.get_match_prediction_features(home_team, away_team).get('features', {})
        except Exception:
            features = {}
        return {col: float(features.get(col, 0.0)) if isinstance(features.get(col), (int, float)) else 0.0 for col in self.feature_columns}
    ...
    def _build_v2_features(self, home_team: str, away_team: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            home_attack = h_seed.get('attack_rating') or self._deterministic_value(f"attack_{h_team['name']}", 1.2, 2.4)
            away_attack = a_seed.get('attack_rating') or self._deterministic_value(f"attack_{a_team['name']}", 1.2, 2.4)
            # defense_rating higher means weaker defense; invert for strength factor
            home_def_raw = h_seed.get('defense_rating') or self._deterministic_value(f"def_{h_team['name']}", 0.6, 1.6)
            away_def_raw = a_seed.get('defense_rating') or self._deterministic_value(f"def_{a_team['name']}", 0.6, 1.6)
            home_def = max(0.2, 2.2 - home_def_raw)
            away_def = max(0.2, 2.2 - away_def_raw)
            home_form = h_seed.get('form_score') or self._deterministic_value(f"form_{h_team['name']}", 0.45, 0.85)
            away_form = a_seed.get('form_score') or self._deterministic_value(f"form_{a_team['name']}", 0.45, 0.85)
            ...
            h2h_avg_goals = round((home_goals_pg + away_goals_pg + home_concede_pg + away_concede_pg)/2, 4)
            ...
            features = {
                ...
                'home_advantage_multiplier': 1.08 + self._deterministic_value(f"hadv_{h_team['name']}", 0.0, 0.07),
    ...
    def _calculate_over_probability(self, total_goals: float) -> float:
        ...
        z_score = (2.5 - total_goals) / max(total_goals * 0.3, 0.5)  # Assume std dev proportional to mean
    ...
    def _calculate_btts_probability(self, features: Dict) -> float:
        ...
        home_scoring_prob = min(0.9, features.get('home_goals_per_game', 1.5) / 3.0 + 0.3)
        away_scoring_prob = min(0.9, features.get('away_goals_per_game', 1.5) / 3.0 + 0.3)
    ...
    def _heuristic_prediction(self, features: Dict) -> Dict[str, Any]:
        ...
        home_expected = features.get('home_goals_per_game', 1.5) * features.get('home_advantage_multiplier', 1.1)
        ...
        try:
            for model_type, models_dict in self.models.items():
                for model_name, model in models_dict.items():
                    if model_name != 'ensemble_weights':
                        model_path = os.path.join(self.model_save_path, f"{model_type}_{model_name}.pkl")
            ...
            if os.path.exists(metadata_path):
                try:
                    ...
                    if (not self.is_trained) and self.training_accuracy:
                        ...
                        try:
                            self._save_models()
                            logger.info("[MODEL LOAD] Auto-repaired is_trained flag based on existing training_accuracy.")
                        except Exception:
                            pass
                    logger.info(f"Models loaded successfully. Training date: {metadata.get('training_date', 'Unknown')}")
                except Exception as e:
                    errors.append((metadata_path, str(e)))
                    loaded_ok = False
            if not loaded_ok:
                logger.warning(f"Model load had errors: {errors}. Proceeding with is_trained=False for fresh retrain.")
                self.is_trained = False
            # Integrity heuristic: if we loaded all expected model pickle files and have training_accuracy, mark trained
            expected_model_files = sum(1 for m in self.models.values() for k in m.keys() if k != 'ensemble_weights')
    ...
    def repair_training_flag(self) -> bool:
        ...
        """
        try:
            expected_model_files = [f"{mt}_{mn}.pkl" for mt, md in self.models.items() for mn in md.keys() if mn != 'ensemble_weights']
            present = 0
            for fname in expected_model_files:
                if os.path.exists(os.path.join(self.model_save_path, fname)):
                    present += 1
            if (not self.is_trained) and self.training_accuracy and present == len(expected_model_files):
                self.is_trained = True
                # Update metadata file so future loads reflect corrected status
                self._save_models()
                logger.info("[MODEL REPAIR] is_trained flag restored based on integrity check.")
            return self.is_trained
        except Exception as e:
    ...
    def get_model_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'ensemble_weights': {k: v.get('ensemble_weights', []) for k, v in self.models.items()},
            'last_training_date': datetime.now().isoformat() if self.is_trained else None,
            'model_version': self.model_version
        }
    ...
    def retrain_and_bump(self, training_data: Optional[pd.DataFrame] = None, level: str = 'patch'):
