...
class HistoricalMLPredictor:
    ...
    async def load_historical_data_and_train(self):
        ...
        try:
            ...
            all_seasons_data = await historical_epl_service.get_comprehensive_historical_data()
            ...
            logger.info(f"📊 Training dataset: {len(training_df)} matches across {len(all_seasons_data)} seasons")
    ...
    def _prepare_comprehensive_training_data(self, all_seasons_data: Dict) -> pd.DataFrame:
    ...
    def _extract_comprehensive_features(self, match: Dict, teams_data: Dict, 
                                      players_data: List[Dict], season: str) -> Optional[Dict]:
        ...
        try:
            ...
            home_top_scorer_goals = self._get_team_top_scorer_goals(home_team, players_data)
            away_top_scorer_goals = self._get_team_top_scorer_goals(away_team, players_data)
            ...
            features = {
                ...
                # Goal averages
                'home_avg_goals_for': home_data.get('goals_for', 0) / max(home_data.get('matches_played', 1), 1),
                'home_avg_goals_against': home_data.get('goals_against', 0) / max(home_data.get('matches_played', 1), 1),
                'away_avg_goals_for': away_data.get('goals_for', 0) / max(away_data.get('matches_played', 1), 1),
                'away_avg_goals_against': away_data.get('goals_against', 0) / max(away_data.get('matches_played', 1), 1),
                ...
                'scorer_goals_difference': home_top_scorer_goals - away_top_scorer_goals,
                ...
                # Advanced metrics
                'home_points_per_match': home_data.get('points', 0) / max(home_data.get('matches_played', 1), 1),
                'away_points_per_match': away_data.get('points', 0) / max(away_data.get('matches_played', 1), 1),
                'goal_difference_gap': home_data.get('goal_difference', 0) - away_data.get('goal_difference', 0),
            ...
            logger.warning(f"Feature extraction failed for {home_team} vs {away_team}: {e}")
            return None
    ...
    def _get_team_top_scorer_goals(self, team_name: str, players_data: List[Dict]) -> int:
    ...
    async def _train_comprehensive_models(self, training_df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            feature_cols = [col for col in training_df.columns 
                          if col not in ['total_goals', 'home_score', 'away_score', 'result', 'over_2_5', 'btts']]
            ...
            success = await self._train_regression_models(X_scaled, y_goals, 'total_goals')
            if success: models_trained += 1
            
            # 2. Match Result Prediction (Classification)
            y_result = training_df['result_encoded']
            success = await self._train_classification_models(X_scaled, y_result, 'match_result')
            if success: models_trained += 1
    ...
    async def _train_regression_models(self, X: np.ndarray, y: pd.Series, target_name: str) -> bool:
        ...
        try:
            ...
            self.training_scores[target_name] = {
                ...
                'mse': mean_squared_error(y_test, self.models[target_name].predict(X_test))
            }
            
            logger.info(f"✅ {target_name} model trained: {model_type} (R² = {best_score:.3f})")
            return True
    ...
    async def _train_classification_models(self, X: np.ndarray, y: pd.Series, target_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ {target_name} model trained: {model_type} (Accuracy = {best_score:.3f})")
            return True
    ...
    async def _train_binary_models(self, X: np.ndarray, y: pd.Series, target_name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ {target_name} model trained: RandomForest (Accuracy = {score:.3f})")
            return True
            
        except Exception as e:
            logger.error(f"❌ Binary classification training failed for {target_name}: {e}")
            return False
    ...
    async def predict_match_comprehensive(self, home_team: str, away_team: str) -> Dict[str, Any]:
        """Generate comprehensive match prediction using historical models"""
        try:
            if not self.is_trained:
                # Try to load pre-trained models
                if not self.load_models():
                    logger.warning("⚠️ No trained models available, using heuristic predictions")
                    return self._generate_heuristic_prediction(home_team, away_team)
            
            # Get current season data for feature extraction
            current_season_data = await historical_epl_service.get_season_data("2025-26")
            
            # Prepare prediction features
            features = await self._prepare_prediction_features(home_team, away_team, current_season_data)
            ...
            if 'total_goals' in self.models:
                ...
                predictions['total_goals_confidence'] = min(0.9, self.training_scores.get('total_goals', {}).get('r2_score', 0.5))
            ...
            if 'match_result' in self.models:
                ...
                predictions['match_result'] = ['home_win', 'away_win', 'draw'][np.argmax(result_probs)]
                predictions['match_result_confidence'] = float(max(result_probs))
            
            # Over/Under 2.5
            if 'over_2_5' in self.models:
                over25_prob = float(self.models['over_2_5'].predict_proba([features])[0][1])
            ...
            predictions['betting_tips'] = self._generate_comprehensive_betting_tips(predictions)
            
            # Add model info
            predictions['prediction_method'] = 'historical_ml_ensemble'
            predictions['models_used'] = list(self.models.keys())
            predictions['training_data_seasons'] = ['2022-23', '2023-24', '2024-25', '2025-26']
    ...
    async def _prepare_prediction_features(self, home_team: str, away_team: str, 
                                         season_data) -> Optional[List[float]]:
    ...
    def _generate_heuristic_prediction(self, home_team: str, away_team: str) -> Dict[str, Any]:
        ...
        team_strength_map = {
            ...
            "Manchester United": 0.75, "Chelsea": 0.75, "Tottenham Hotspur": 0.7,
            "Newcastle United": 0.7, "Brighton & Hove Albion": 0.65,
            "Aston Villa": 0.65, "West Ham United": 0.6
        }
        ...
        return {
            ...
            'match_result': 'home_win' if home_win_prob > max(draw_prob, away_win_prob) else ('away_win' if away_win_prob > draw_prob else 'draw'),
            ...
            'betting_tips': [
                {'market': 'Match Result', 'recommendation': 'Home Win' if home_win_prob > 0.5 else 'Draw', 'reasoning': 'Based on heuristic team strength'},
                {'market': 'Total Goals', 'recommendation': f'Over 2.5' if total_goals > 2.5 else 'Under 2.5', 'reasoning': f'Predicted goals: {total_goals:.1f}'}
            ],
