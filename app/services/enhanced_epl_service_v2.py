...
import logging
from .team_seed_metrics import team_seed_metrics_service, FEATURE_VERSION as TEAM_FEATURE_VERSION
...
class EnhancedEPLService:
    ...
    def get_matches(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        for fixture in fixtures:
            ...
            try:
                ...
                if not v or str(v).strip().lower() in ('unknown', 'tbd', 'n/a', 'none', ''):
                    # Map using home team
                    home_name = (enhanced_fixture.get('home_team') or enhanced_fixture.get('homeTeam') or
                                 (enhanced_fixture.get('home', {}) or {}).get('name'))
            ...
            home_score = (enhanced_fixture.get('home_score') or enhanced_fixture.get('homeScore') or
                          enhanced_fixture.get('full_time_home_goals') or enhanced_fixture.get('home_goals'))
            away_score = (enhanced_fixture.get('away_score') or enhanced_fixture.get('awayScore') or
                          enhanced_fixture.get('full_time_away_goals') or enhanced_fixture.get('away_goals'))
            ...
            home_team_name = (enhanced_fixture.get('home_team') or enhanced_fixture.get('homeTeam')
                              or (enhanced_fixture.get('home', {}) or {}).get('name'))
            away_team_name = (enhanced_fixture.get('away_team') or enhanced_fixture.get('awayTeam')
                              or (enhanced_fixture.get('away', {}) or {}).get('name'))
            c_actual = corners_actuals_store.lookup(date_str, home_team_name, away_team_name)
            ...
            enhanced_fixture.update({
                ...
                'total_corners': c_actual.get('total_corners') if c_actual else None,
    ...
    def refresh_data(self) -> Dict[str, Any]:
        ...
        try:
            self.football_data = self._load_football_data()
            fixtures_count = len(self.football_data.get('converted_fixtures', [])) if self.football_data else 0
            return {"reloaded": True, "fixtures": fixtures_count, "season": self.current_season}
        except Exception as e:
    ...
    def get_players_goals(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        goal_scorers = [
            {'name': 'Erling Haaland', 'team': 'Manchester City', 'goals': 8, 'expected_season_total': 28},
            {'name': 'Mohamed Salah', 'team': 'Liverpool', 'goals': 7, 'expected_season_total': 24},
            {'name': 'Harry Kane', 'team': 'Bayern Munich', 'goals': 6, 'expected_season_total': 22},
            {'name': 'Alexander Isak', 'team': 'Newcastle United', 'goals': 6, 'expected_season_total': 18},
            {'name': 'Cole Palmer', 'team': 'Chelsea', 'goals': 5, 'expected_season_total': 16},
            {'name': 'Bryan Mbeumo', 'team': 'Brentford', 'goals': 5, 'expected_season_total': 14},
            {'name': 'Bukayo Saka', 'team': 'Arsenal', 'goals': 4, 'expected_season_total': 15},
            {'name': 'Ollie Watkins', 'team': 'Aston Villa', 'goals': 4, 'expected_season_total': 16},
            {'name': 'Dominic Solanke', 'team': 'Bournemouth', 'goals': 4, 'expected_season_total': 12},
            {'name': 'Chris Wood', 'team': 'Nottingham Forest', 'goals': 4, 'expected_season_total': 11},
            {'name': 'Liam Delap', 'team': 'Ipswich Town', 'goals': 3, 'expected_season_total': 8},
            {'name': 'Antoine Semenyo', 'team': 'Bournemouth', 'goals': 3, 'expected_season_total': 9},
            {'name': 'Joao Pedro', 'team': 'Brighton', 'goals': 3, 'expected_season_total': 12},
            {'name': 'Kai Havertz', 'team': 'Arsenal', 'goals': 3, 'expected_season_total': 13},
            {'name': 'Nicolas Jackson', 'team': 'Chelsea', 'goals': 3, 'expected_season_total': 14}
        ]
        
        # Add enhanced stats
        for player in goal_scorers:
            player.update({
                'goals_per_game': round(player['goals'] / max(8, 1), 2),  # Assuming 8 games played
                'expected_goals_per_game': round(player['expected_season_total'] / 38, 2),
                'form_rating': np.random.uniform(0.6, 1.0),  # Mock form rating
                'injury_risk': np.random.choice(['low', 'medium', 'high'], p=[0.7, 0.2, 0.1]),
                'season': self.current_season
            })
            ...
            if fixture['status'] == 'SCHEDULED':
                try:
                    ...
                    if date_str.endswith('Z'):
                        ...
                        match_date = datetime.fromisoformat(date_str.replace('UTC', '+00:00'))
                    else:
                    ...
                    if 0 <= days_until <= days:
                        ...
                        enhanced_fixture['prediction_confidence'] = 0.85  # Mock confidence
    ...
    def get_detailed_team_stats(self, team_name: Optional[str] = None) -> Dict:
        ...
        if team_name:
            ...
            stats = {
                ...
                'matches_played': len([f for f in self.get_matches() if f['status'] == 'FINISHED']),
    ...
    def _get_fallback_teams(self) -> List[Dict]:
        ...
        return [
            {'name': 'Arsenal', 'position': 1, 'points': 0, 'season': self.current_season},
            {'name': 'Liverpool', 'position': 2, 'points': 0, 'season': self.current_season},
            {'name': 'Manchester City', 'position': 3, 'points': 0, 'season': self.current_season},
            # ... rest of EPL teams
        ]
        ...
        return [
            {
                ...
                'date': '2025-09-28', 'status': 'SCHEDULED',
                'season': self.current_season
            }
        ]
    ...
    def get_system_status(self) -> Dict:
        ...
        if self.football_data:
            status.update({
                ...
                'scheduled_fixtures': self.football_data.get('scheduled_fixtures', 0),
