...
class ModelReconciliationService:
    ...
    def reconcile_match_predictions(
        self, match: Dict, predictions: Dict
    ) -> Dict[str, Any]:
        ...
        predicted_corners_total = _to_float(
            (predictions or {}).get("total_corners", 0.0), 0.0
        )
        ...
        yh, yd, ya = (
            (1 if actual_result == "H" else 0),
            (1 if actual_result == "D" else 0),
            (1 if actual_result == "A" else 0),
        )
        ...
        corners_line = 9.5
        goals_pred_over = (
            None if predicted_total is None else (predicted_total > goals_line)
        )
        ...
        corners_pred_over = (
            None
            if (
                predicted_corners_total is None
                or (
                    predicted_corners_total == 0.0
                    and (predictions or {}).get("total_corners") is None
                )
            )
            else (predicted_corners_total > corners_line)
        )
        corners_actual_over = (
            None if (actual_tc is None) else (actual_tc > corners_line)
        )
        ...
        reconciliation = {
            ...
            "predicted": {
                ...
                "total_corners": predicted_corners_total
                if predicted_corners_total
                else None,
    ...
    def calculate_weekly_model_performance(
        ...
        return {
            ...
            "goals_ou_accuracy": (goals_ou_correct / goals_ou_total)
            if goals_ou_total
            else None,
            "corners_ou_accuracy": (corners_ou_correct / corners_ou_total)
            if corners_ou_total
            else None,
        }
