...
class EnhancedEPLService:
    ...
    def get_detailed_team_stats(self, team_name: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            for _, team in teams_data.iterrows():
                ...
                name = get_val(
                    team, ["Team", "Club", "Name", "team", "club", "name"], "Unknown"
                )
                played = (
                    get_val(team, ["Played", "P", "Matches", "MP", "played"], 0) or 0
                )
                ...
                gf = (
                    get_val(
                        team, ["Goals For", "GF", "For", "GoalsFor", "goals_for"], 0
                    )
                    or 0
                )
                ga = (
                ...
                gd = get_val(
                    team,
                    ["Goal Difference", "GD", "Diff", "GoalDiff", "goal_difference"],
                    None,
                )
                ...
                for key_name in (
                    ...
                    try:
                        ...
                        locals()[key_name] = (
                            int(float(locals()[key_name]))
                            if str(locals()[key_name]).replace(".", "", 1).isdigit()
                            else 0
                        )
                try:
                ...
                team_stats = {
                    ...
                    "points_per_game": (float(points) / max(int(played), 1))
                    if isinstance(points, (int, float))
                    else 0.0,
                    "goals_per_game": (float(gf) / max(int(played), 1))
                    if isinstance(gf, (int, float))
                    else 0.0,
                    "goals_conceded_per_game": (float(ga) / max(int(played), 1))
                    if isinstance(ga, (int, float))
                    else 0.0,
                    "win_rate": (float(won) / max(int(played), 1))
            ...
            if not self._team_stats_warned_once:
                logger.warning(
                    f"Error getting detailed team stats (using fallback once): {e}"
                )
    ...
    def get_match_prediction_features(self, home_team: str, away_team: str) -> Dict:
        ...
        try:
            ...
            teams_list = (
                team_stats.get("teams", []) if isinstance(team_stats, dict) else []
            )
            home_stats = next(
                (
                    t
                    for t in teams_list
                    if str(t.get("name", "")).lower() == home_team.lower()
                ),
                None,
            )
            away_stats = next(
                (
                    t
                    for t in teams_list
                    if str(t.get("name", "")).lower() == away_team.lower()
                ),
                None,
            )
            ...
            features = {
                ...
                # Advanced metrics
                "goal_difference_gap": home_stats["goal_difference"]
                - away_stats["goal_difference"],
                "position_difference": away_stats["position"]
                - home_stats["position"],  # Negative favors home
                ...
                # Match context
                "is_top_6_clash": home_stats["position"] <= 6
                and away_stats["position"] <= 6,
                "is_relegation_battle": home_stats["position"] >= 15
                and away_stats["position"] >= 15,
                "strength_disparity": abs(
            ...
            logger.error(
                f"Error extracting match features for {home_team} vs {away_team}: {e}"
            )
            return self._get_fallback_prediction_features()

    def get_head_to_head_stats(
        self, home_team: str, away_team: str, last_n: int = 5
    ) -> Dict:
    ...
    def get_betting_odds_data(self) -> Dict:
        ...
        try:
            ...
            top_teams = sorted(
                teams_list, key=lambda x: x.get("points", 0), reverse=True
            )[:6]
            ...
            return {
                "title_odds": [
                    {
                        ...
                        "probability": self._calculate_title_probability(
                            team, top_teams
                        ),
                        "form_trend": "up"
                        if team["form_score"] > 0.6
                        else "down"
                        if team["form_score"] < 0.4
                        else "stable",
                    }
                ...
                "market_insights": {
                    "highest_scoring_team": max(
                        teams_list, key=lambda x: x.get("goals_per_game", 0)
                    )["name"]
                    if teams_list
                    else "N/A",
                    "best_defense": min(
                        teams_list, key=lambda x: x.get("goals_conceded_per_game", 999)
                    )["name"]
                    if teams_list
                    else "N/A",
                    "most_consistent": max(
                        teams_list, key=lambda x: x.get("scoring_consistency", 0)
                    )["name"]
                    if teams_list
                    else "N/A",
                    "league_avg_goals": (
                        team_stats.get("league_averages", {}) or {}
                    ).get("goals_per_game", 2.7)
    ...
    def _calculate_clean_sheets(self, team: pd.Series) -> float:
        ...
        return max(
            0.3 - (team.get("Goals Against", 20) / max(team.get("Played", 1), 1)) * 0.1,
            0.0,
        )
    ...
    def _calculate_defensive_strength(self, team: pd.Series) -> float:
        ...
        goals_conceded_per_game = team.get("Goals Against", 30) / max(
            team.get("Played", 1), 1
        )
    ...
    def _calculate_league_averages(self, teams_data: List[Dict]) -> Dict:
        ...
        if not teams_data:
            return {"goals_per_game": 2.7, "points_per_game": 1.5, "win_rate": 0.33}
        ...
        return {
            ...
            "goals_conceded_per_game": np.mean(
                [t["goals_conceded_per_game"] for t in teams_data]
            ),
        }
    ...
    def _calculate_title_odds(self, team: Dict, top_teams: List[Dict]) -> float:
        ...
        raw_odds = (
            1.5 + (position_factor * 0.5) + (points_factor * 0.3) + (form_factor * 0.2)
        )
    ...
    def _calculate_title_probability(self, team: Dict, top_teams: List[Dict]) -> float:
        ...
        probability = (
            position_weight * 0.4 + form_weight * 0.3 + points_weight * 0.3
        ) * 100
        ...
        try:
            ...
            for _, player in goal_scorers.head(10).iterrows():
                player_data = {
                    ...
                    "probability": self._calculate_scoring_probability(
                        player.get("Goals", 0)
                    ),
                }
                ...
                relegation_candidates.append(
                    {
                        ...
                        "points": team["points"],
                        "relegation_odds": self._calculate_relegation_odds(
                            relegation_risk
                        ),
                        "relegation_probability": relegation_risk * 100,
                        "risk_level": "High"
                        if relegation_risk > 0.6
                        else "Medium"
                        if relegation_risk > 0.3
                        else "Low",
                    }
                )
    ...
    def _generate_season_prediction(self, teams: List[Dict]) -> Dict:
        ...
        return {
            ...
            "champion_probability": self._calculate_title_probability(
                top_team, teams[:6]
            ),
            "predicted_final_points": int(
                top_team["points"]
                + (top_team["points_per_game"] * (38 - top_team["matches_played"]))
            ),
            "top_4_prediction": [
                team["name"]
                for team in sorted(teams, key=lambda x: x["points"], reverse=True)[:4]
            ],
            "relegation_prediction": [
                team["name"] for team in sorted(teams, key=lambda x: x["points"])[:3]
            ],
            "season_goals_prediction": sum(team["goals_for"] for team in teams)
            + int(
                sum(team["goals_per_game"] for team in teams)
    ...
    def _get_fallback_betting_data(self) -> Dict:
        ...
        return {
            ...
            "top_scorer_odds": [],
            "relegation_odds": [],
            "market_insights": {"data_source": "fallback", "league_avg_goals": 2.7},
        }
    ...
    def _calculate_scoring_probability(self, current_goals: int) -> float:
        ...
        total_goals_projection = current_goals * (
            38 / max(25, 1)
        )  # Assume 25 games played
