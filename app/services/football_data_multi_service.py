...
class FootballDataMultiService:
    def __init__(self) -> None:
        self.api_key = os.getenv('FOOTBALL_DATA_API_KEY') or os.getenv('FOOTBALL_DATA_API_TOKEN')
        if not self.api_key:
    ...
    def _convert_fixtures(self, fixtures: List[Dict[str, Any]], competition_name: str) -> List[Dict[str, Any]]:
    ...
    def cache_league(self, code: str) -> Dict[str, Any]:
        ...
        if comp.get('error'):
            return {'success': False, 'error': comp.get('error'), 'body': comp.get('body')}
        comp_name = comp.get('name') or code
        # 2. Teams
        teams = self._get(f"competitions/{code}/teams", params={'season': SEASON_YEAR}).get('teams', [])
        # 3. Matches (fixtures)
        matches = self._get(f"competitions/{code}/matches", params={'season': SEASON_YEAR}).get('matches', [])
        ...
        return {'success': True, 'file': path, 'fixtures': len(matches), 'teams': len(teams)}
