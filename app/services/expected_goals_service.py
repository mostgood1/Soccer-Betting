...
class ExpectedGoalsService:
    ...
    def _prepare(self):
        try:
            ...
            for m in matches:
                ...
                rec_h = agg.setdefault(h, {'home_goals':0,'home_conc':0,'home_matches':0,'away_goals':0,'away_conc':0,'away_matches':0})
                rec_a = agg.setdefault(a, {'home_goals':0,'home_conc':0,'home_matches':0,'away_goals':0,'away_conc':0,'away_matches':0})
                rec_h['home_goals'] += hg
                rec_h['home_conc'] += ag
            ...
            if lambda_home_league <=0: lambda_home_league = 1.4
            if lambda_away_league <=0: lambda_away_league = 1.2
            self.league_avgs = {
    ...
    def get_expected_goals(self, home_team: str, away_team: str) -> Dict[str, Any]:
        ...
        if not hm:
            hm = {'attack_home':1.0,'attack_away':1.0,'defense_home':1.0,'defense_away':1.0}
        if not am:
            am = {'attack_home':1.0,'attack_away':1.0,'defense_home':1.0,'defense_away':1.0}
    ...
    def scoreline_distribution(self, lambda_home: float, lambda_away: float, max_goals: int = 6) -> Dict[str, Any]:
        matrix = []
        prob_total = 0.0
        ...
        if scale > 0:
            ...
            p_away /= scale
        # Over/Under 2.5
        over_25 = sum(matrix[i][j] for i in range(len(matrix)) for j in range(len(matrix)) if i+j >= 3)
        under_25 = 1 - over_25
        return {
            'matrix': matrix,
    ...
    def get_full_xg(self, home_team: str, away_team: str, max_goals: int = 6) -> Dict[str, Any]:
        eg = self.get_expected_goals(home_team, away_team)
        dist = self.scoreline_distribution(eg['lambda_home'], eg['lambda_away'], max_goals=max_goals)
