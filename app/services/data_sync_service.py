...
class DataSyncService:
    ...
    def sync_epl_teams(self, season: str = "2024-25") -> int:
        ...
        try:
            ...
            epl_league = (
                ...
                .filter(League.name == "Premier League", League.season == season)
                .first()
            )
            ...
            for _, team_row in teams_df.iterrows():
                team_name = team_row.get(
                    "Squad",
                    team_row.name if hasattr(team_row, "name") else str(team_row),
                )

                # Check if team already exists
                existing_team = (
                    self.db.query(Team)
                    .filter(Team.name == team_name, Team.league_id == epl_league.id)
                    .first()
                )
            ...
            for _, match_row in matches_df.iterrows():
                try:
                    ...
                    home_team = (
                        self.db.query(Team)
                        .filter(
                            Team.name == home_team_name, Team.league_id == epl_league.id
                        )
                        .first()
                    )
                    ...
                    away_team = (
                        self.db.query(Team)
                        .filter(
                            Team.name == away_team_name, Team.league_id == epl_league.id
                        )
                        .first()
                    )
                    ...
                    if existing_match:
                        ...
                        if "Score" in match_row and pd.notna(match_row["Score"]):
                            ...
                            if "–" in score_str or "-" in score_str:
                                home_score, away_score = score_str.replace(
                                    "–", "-"
                                ).split("-")
                                existing_match.home_score = int(home_score)
                                existing_match.away_score = int(away_score)
                                existing_match.total_goals = int(home_score) + int(
                                    away_score
                                )
    ...
    def sync_player_stats(self, season: str = "2024-25") -> int:
        ...
        try:
            # Get player stats
            player_stats_df = self.soccer_data_service.get_player_statistics(
                season, "standard"
            )
            ...
            for _, player_row in player_stats_df.iterrows():
                try:
                    ...
                    existing_player = (
                        ...
                        .filter(Player.name == player_name, Player.team_id == team.id)
                        .first()
                    )
                    ...
                    if not existing_player:
                        ...
                        existing_player.season_appearances = int(
                            player_row.get("MP", 0)
                        )
                        existing_player.season_minutes_played = int(
                            player_row.get("Min", 0)
                        )
