...
class PredictionService:
    def __init__(self, db: Session):
        ...
        if not self.advanced_predictor.is_trained:
            ...
            if not model_loaded:
                logger.info(
                    "No pre-trained models found. Will use heuristic predictions until models are trained."
                )

    def generate_prediction(
        self, match_id: int, model_version: str = "v2.0", use_advanced_ml: bool = True
    ) -> Prediction:
        ...
        try:
            ...
            prediction = Prediction(
                ...
                first_half_goals_confidence=predictions.get(
                ...
                total_corners_confidence=predictions.get(
                    "total_corners_confidence", 0.5
                ),
            ...
            logger.info(
                f"Generated {algorithm_used} prediction for match {match_id} with model {model_version}"
            )
            return prediction
    ...
    def generate_real_time_prediction(
        self, home_team: str, away_team: str, model_version: str = "v2.0"
    ) -> Dict[str, Any]:
        """Generate real-time prediction for any team matchup without requiring database match"""
        try:
            logger.info(
                f"Generating real-time prediction for {home_team} vs {away_team}"
            )

            if self.advanced_predictor.is_trained:
                predictions = self.advanced_predictor.predict_match(
                    home_team, away_team
                )

                # Add betting insights
                predictions["betting_tips"] = self._generate_betting_tips(predictions)
                predictions["confidence_rating"] = self._calculate_overall_confidence(
                    predictions
                )
    ...
    def _generate_simple_prediction(
        self, features_data: Dict, home_team: str, away_team: str
    ) -> Dict[str, Any]:
        """Generate simple prediction when advanced ML is not available"""
        features = features_data.get("features", {})

        home_goals_expected = features.get("home_goals_per_game", 1.5) * features.get(
        ...
        return {
            ...
            if home_goals_expected > away_goals_expected
            else 0.35,
            ...
            "match_result": "H"
            if home_goals_expected > away_goals_expected * 1.1
            else "A"
            if away_goals_expected > home_goals_expected * 1.1
            else "D",
    ...
    def train_advanced_models(self, force_retrain: bool = False) -> Dict[str, Any]:
        ...
        try:
            if self.advanced_predictor.is_trained and not force_retrain:
                logger.info(
                    "Models already trained. Use force_retrain=True to retrain."
                )
                return {
    ...
    def evaluate_prediction(self, prediction_id: int) -> Dict[str, Any]:
        ...
        prediction = (
            self.db.query(Prediction).filter(Prediction.id == prediction_id).first()
        )
        if not prediction:
        ...
        if prediction.total_goals_prediction and match.total_goals is not None:
            ...
            accuracy_results["goals_accuracy"] = max(
                0, 1 - (goals_diff / 10)
            )  # Scale accuracy
        ...
        if prediction.first_half_goals_prediction and match.total_goals_ht is not None:
            ht_goals_diff = abs(
                prediction.first_half_goals_prediction - match.total_goals_ht
            )
        ...
        if prediction.match_result_prediction and match.result:
            accuracy_results["result_accuracy"] = (
                1.0 if prediction.match_result_prediction == match.result else 0.0
            )
        ...
        logger.info(
            f"Evaluated prediction {prediction_id} with overall accuracy {overall_accuracy}"
        )
    ...
    def get_prediction_statistics(self, model_version: str = None) -> Dict[str, Any]:
        ...
        avg_accuracy = (
            sum([p.accuracy_score for p in predictions if p.accuracy_score])
            / total_predictions
        )
