...
def _extract_match_level_odds(match_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if total_overround_samples:
        out['raw_overround'] = sum(total_overround_samples)/len(total_overround_samples)
    return out


def fetch_week_odds(competition: str, season: int, matchday: int, deep_lookup: bool = False, token: str | None = None, throttle_seconds: float = 0.4) -> Dict[str, Any]:
    ...
    try:
        ...
        avg_overround = None
        over_samples = [r['raw_overround'] for r in out_rows if isinstance(r.get('raw_overround'), (float,int))]
        if over_samples:
