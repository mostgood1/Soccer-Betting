...
class CornersMarketStore:
    ...
    def _ingest_row(self, row: Dict[str, Any]) -> None:
        try:
            ...
            rec = CornersMarketRecord(
                ...
                home=home,
                away=away,
                market_type="team" if mkt.startswith("team") else "total",
    ...
    def _closest_line(
        self, mapping: Dict[float, CornersMarketRecord], target_line: float
    ) -> Optional[CornersMarketRecord]:
        if not mapping:
            return None
    ...
    def lookup_team(
        self, date: str, home: str, away: str, side: str, line: float
    ) -> Optional[Tuple[float, Optional[str], float]]:
...
def reload_market_store() -> Tuple[int, int]:
    ...
    return (
        sum(len(v) for v in corners_market_store._totals.values()),
