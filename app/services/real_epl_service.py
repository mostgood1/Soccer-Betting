...
class RealEPLService:
    def __init__(self):
        ...
        self.ranking_table = RankingTable(league=self.league, target_season=self.current_season)
    ...
    def get_epl_teams(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"Error getting real teams: {str(e)}")  # Use str() to avoid encoding issues
            return self._get_fallback_teams()
        ...
        try:
            ...
            for game in games_data[:10]:  # Limit to 10 matches
                ...
                match = {
                    ...
                    'corners': self._extract_corners_from_stats(game.get('game_stats', [])),
                    'cards': self._extract_cards_from_stats(game.get('game_stats', [])),
                    'shots_home': self._extract_shots_from_stats(game.get('game_stats', []), 'home'),
                    'shots_away': self._extract_shots_from_stats(game.get('game_stats', []), 'away'),
                    'possession_home': self._extract_possession_from_stats(game.get('game_stats', []), 'home'),
                    'possession_away': self._extract_possession_from_stats(game.get('game_stats', []), 'away'),
                }
    ...
    def get_betting_predictions(self) -> Dict:
        ...
        return {
            'featured_bets': [
                {
                    'match': f"{recent_matches[0]['home_team']} vs {recent_matches[0]['away_team']}" if recent_matches else "Liverpool vs Arsenal",
                    'prediction': 'Over 2.5 Goals',
                    'odds': 1.85,
                    'confidence': 78
                },
                {
                    'match': f"{recent_matches[1]['home_team']} vs {recent_matches[1]['away_team']}" if len(recent_matches) > 1 else "Man City vs Chelsea", 
            ...
            'goal_predictions': {
                'high_scoring_matches': [match['home_team'] + ' vs ' + match['away_team'] 
                                       for match in recent_matches if match['total_goals'] > 2][:3],
                'average_goals_per_match': sum(match['total_goals'] for match in recent_matches) / len(recent_matches) if recent_matches else 2.7
            }
        }
    ...
    def _extract_shots_from_stats(self, game_stats: List[Dict], team: str) -> int:
        ...
        try:
            team_stats = game_stats[0] if team == 'home' else game_stats[1] if len(game_stats) > 1 else game_stats[0]
            return (team_stats.get('shots_total_FW', 0) + 
    ...
    def _get_fallback_matches(self) -> List[Dict]:
        ...
        for i in range(10):
            ...
            matches.append({
                ...
                'first_half_goals': random.randint(0, min(2, home_score + away_score)),
            })
    ...
    def _get_fallback_predictions(self) -> Dict:
        ...
        return {
            'featured_bets': [
                {'match': 'Arsenal vs Liverpool', 'prediction': 'Over 2.5 Goals', 'odds': 1.85, 'confidence': 75},
                {'match': 'Man City vs Chelsea', 'prediction': 'Both Teams to Score', 'odds': 1.65, 'confidence': 80}
            ],
            'top_teams_odds': [
            ...
            'goal_predictions': {
                'high_scoring_matches': ['Arsenal vs Liverpool', 'Man City vs Tottenham'],
