...
class SoccerDataService:
    ...
    def get_epl_teams(self) -> List[Dict]:
        ...
        if self.connected:
            try:
                ...
                if not schedule.empty:
                    ...
                    if teams:
                        return [
                            {"name": str(team), "league": "Premier League"}
        ...
        return [
            {"name": team, "league": "Premier League", "country": "England"}
                ...
                if not schedule.empty:
                    ...
                    for idx, match in schedule.iterrows():
                        match_dict = {
                            ...
                            "status": "completed"
                            if pd.notna(match.get("home_score"))
                            else "scheduled",
                        }
    ...
    def calculate_team_form(self, team_name: str, num_matches: int = 5) -> Dict:
        ...
        for match in matches:
            if (
                match.get("home_team") == team_name
                or match.get("away_team") == team_name
            ):
