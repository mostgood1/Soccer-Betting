...
class ReconciliationStore:
    ...
    def compute_aggregates(self):
        ...
        def _agg(list_r: List[Dict[str, Any]]):
            ...
            res_correct = sum(
                1 for r in list_r if _as_bool(r["accuracy"].get("result_correct"))
            )
            goals_w1 = sum(
                1 for r in list_r if _as_bool(r["accuracy"].get("goals_within_1"))
            )
            goals_w2 = sum(
                1 for r in list_r if _as_bool(r["accuracy"].get("goals_within_2"))
            )
            avg_total_diff = (
                sum(r["accuracy"].get("total_goals_diff", 0) for r in list_r) / total
            )
            avg_home_diff = (
                sum(r["accuracy"].get("home_goals_diff", 0) for r in list_r) / total
            )
            avg_away_diff = (
                sum(r["accuracy"].get("away_goals_diff", 0) for r in list_r) / total
            )
            # Brier score average if present
            briers = [
                r.get("brier_score")
                for r in list_r
                if isinstance(r.get("brier_score"), (float, int))
            ]
            ...
            for r in list_r:
                ...
                if None not in (ph, pd, pa):
                    ...
                    sharpness_vals.append(
                        abs(ph - 1 / 3) + abs(pd - 1 / 3) + abs(pa - 1 / 3)
                    )
            avg_sharpness = (
                sum(sharpness_vals) / len(sharpness_vals) if sharpness_vals else None
            )
            return {
                "matches": total,
                ...
                "prob_sharpness": round(avg_sharpness, 4)
                if avg_sharpness is not None
                else None,
            }
