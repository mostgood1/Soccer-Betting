...
class WeekSnapshotService:
    ...
    def capture_closing(self, week: int, force: bool = False) -> Dict[str, Any]:
        ...
        for m in matches:
            ...
            normalized_input = {'home_odds': home_odds, 'draw_odds': draw_odds, 'away_odds': away_odds}
            ...
            rows.append({
                ...
                'model_result_confidence': model_pred.get('match_result_confidence'),
        ...
        self._closing_path(week).write_text(json.dumps(snapshot, indent=2), encoding='utf-8')
        return {"status": "created", "week": week, "snapshot": snapshot}

    # --------------- Finalize Week ---------------
    def finalize_week(self, week: int, retrain: bool = False, version_bump: str = 'minor', edge_threshold: float = 0.05) -> Dict[str, Any]:
        ...
        for row in closing['rows']:
            ...
            is_completed = m.get('status') in ['FINISHED','COMPLETED'] or m.get('is_completed') or (home_score is not None and away_score is not None)
            actual_result = None
        ...
        if retrain and completed > 0:
            ...
            new_version = advanced_ml_predictor.retrain_and_bump(training_data=combined, level=version_bump)
            retrain_info = {
        ...
        self._final_path(week).write_text(json.dumps(final_snapshot, indent=2), encoding='utf-8')
        return {"status": "finalized", "week": week, "final": final_snapshot, "retrain": retrain_info}

    def simulate_finalize_week(self, week: int, persist: bool = False, edge_threshold: float = 0.05) -> Dict[str, Any]:
        ...
        if real_final:
            return {"status": "already-finalized-real", "week": week, "final": real_final}
        simulated_rows = []
        for row in closing['rows']:
            r = dict(row)
            ...
            if pick == 'H':
                home_score = max(1, int(round(est_total * 0.6 + random.uniform(-0.3,0.3))))
                away_score = max(0, int(round(est_total - home_score + random.uniform(-0.3,0.3))))
            elif pick == 'A':
                away_score = max(1, int(round(est_total * 0.55 + random.uniform(-0.3,0.3))))
                home_score = max(0, int(round(est_total - away_score + random.uniform(-0.3,0.3))))
            else:  # Draw
                home_score = max(0, int(round(est_total/2 + random.uniform(-0.3,0.3))))
        ...
        if persist:
            ...
            if not self.load_final(week):
                self._final_path(week).write_text(json.dumps(snapshot, indent=2), encoding='utf-8')
        return {"status": "simulated", "week": week, "final": snapshot, "persisted": persist}
    ...
    def build_report(self, week: int) -> Dict[str, Any]:
        ...
        if baseline and closing:
            ...
            for r in closing.get('rows', []):
                ...
                close_index[mk] = r
            deltas = []
            agg_shift = {'H':0.0,'D':0.0,'A':0.0}; counted=0
            for p in baseline.get('predictions', []):
                ...
                valid=True
                for k_map in [('H','home_win_prob'),('D','draw_prob'),('A','away_win_prob')]:
                    k, bp = k_map
            ...
            if counted:
                ...
                avg_shift = None
            baseline_deltas = {'matches_compared': counted, 'average_shift': avg_shift, 'details': deltas}
        return {
            'week': week,
    ...
    def _normalize_market_odds(self, odds: Dict[str, Any]) -> Optional[Dict[str, float]]:
        ...
        """
        try:
            h = odds.get('home_odds'); d = odds.get('draw_odds'); a = odds.get('away_odds')
            if not all([h, d, a]):
                return None
    ...
    def _compute_metrics(self, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for r in eval_rows:
            ...
            if edge_sel and edge_sel.get('selected_outcome') and edge_sel.get('decimal_odds'):
        ...
        return {
            ...
            'realized_roi_avg': (realized_roi_sum/realized_roi_count) if realized_roi_count else None,
    ...
    # --------------- Edge Analytics ---------------
    def _compute_edge_analytics(self, rows: List[Dict[str, Any]], edge_threshold: float = 0.05):
        ...
        for r in rows:
            ...
            details = []
            for outcome, odd_key in [('H','home_odds'),('D','draw_odds'),('A','away_odds')]:
            ...
            # Choose selection
            candidates = [d for d in details if d['edge'] is not None and d['edge'] >= edge_threshold and d['decimal_odds']]
            selection = None
            if candidates:
        ...
        agg = {
            ...
            'expected_roi_avg': (expected_roi_sum/selection_count) if selection_count else None
        }
