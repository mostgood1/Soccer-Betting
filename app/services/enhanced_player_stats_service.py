...
class EnhancedPlayerStatsService:
    ...
    def _generate_player_season_stats(self, player: Dict) -> Dict:
        ...
        return {
            ...
            "minutes_per_assist": int(minutes_played / max(assists, 1))
            if assists > 0
            else 0,
            "shot_accuracy": round(shots_on_target / max(shots, 1), 3)
            if shots > 0
            else 0,
            ...
            **goalkeeper_stats,
            ...
            "injury_status": np.random.choice(
                ["fit", "minor_knock", "injured"], p=[0.85, 0.12, 0.03]
            ),
    ...
    def get_top_assisters(self, limit: int = 20) -> List[Dict]:
        ...
        return sorted(self.players_data, key=lambda x: x["assists"], reverse=True)[
            :limit
        ]
    ...
    def get_top_performers(
        self, metric: str = "average_rating", limit: int = 15
    ) -> List[Dict]:
        ...
        return sorted(self.players_data, key=lambda x: x.get(metric, 0), reverse=True)[
            :limit
        ]
    ...
    def get_disciplinary_stats(self) -> Dict:
        ...
        return {
            ...
            "average_yellows_per_player": round(
                total_yellows / len(self.players_data), 2
            ),
    ...
    def get_goalkeeper_stats(self) -> List[Dict]:
        ...
        return sorted(
            goalkeepers, key=lambda x: x.get("save_percentage", 0), reverse=True
        )
    ...
    def get_stats_summary(self) -> Dict:
        ...
        total_goals = (
            sum(p["goals"] for p in self.players_data) if self.players_data else 0
        )
        total_assists = (
            sum(p["assists"] for p in self.players_data) if self.players_data else 0
        )
        ...
        return {
            ...
            "average_goals_per_player": round(total_goals / total_players, 2)
            if total_players
            else 0,
            "average_assists_per_player": round(total_assists / total_players, 2)
            if total_players
            else 0,
