    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for r in reader:
                ...
                try:
                    home_goals = (
                        int(r.get("FTHG")) if r.get("FTHG") not in (None, "") else None
                    )
                except Exception:
                    home_goals = None
                try:
                    away_goals = (
                        int(r.get("FTAG")) if r.get("FTAG") not in (None, "") else None
                    )
                except Exception:
                    away_goals = None
                ...
                if entry["bookmakers"]:
                    ...
                    if any(prob_samples[k] for k in prob_samples):
                        entry["consensus_implied"] = {
                            k: (median(prob_samples[k]) if prob_samples[k] else None)
                            for k in prob_samples
                        }
                        entry["consensus_overround"] = (
                            median(over_samples) if over_samples else None
                        )
                    ...
                    for bm_key, bm_data in entry["bookmakers"].items():
                        ...
                        if not isinstance(t, dict):
                            continue
                        ov = t.get("over_implied")
                        un = t.get("under_implied")
                        oor = t.get("overround")
                        if isinstance(ov, (int, float)):
                    ...
                    if tot_over_samples and tot_under_samples:
                        entry["consensus_totals_2_5"] = {
                            ...
                            "overround": round(median(tot_ovr_overrounds), 6)
                            if tot_ovr_overrounds
                            else None,
                        }
                rows.append(entry)
...
def ingest_path(path: str) -> Dict[str, Any]:
    ...
    overround_mean = (
        round(sum(overround_samples) / len(overround_samples), 4)
