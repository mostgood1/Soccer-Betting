...
class FootballDataService:
    ...
    def __init__(self):
        ...
        print(
            f"Football-Data.org Service initialized for {self.season}-{int(self.season)+1} season"
        )
    ...
    def get_teams(self) -> List[Dict]:
        ...
        print(
            f"Retrieved {len(teams)} EPL teams for {self.season}-{int(self.season)+1} season"
        )
        return teams
    ...
    def get_fixtures(self, status: str = None) -> List[Dict]:
        ...
        print(
            f"Retrieved {len(fixtures)} EPL fixtures for {self.season}-{int(self.season)+1} season"
        )
        return fixtures
    ...
    def convert_to_epl_format(self, fixtures: List[Dict]) -> List[Dict]:
        ...
        for fixture in fixtures:
            ...
            converted_match = {
                ...
                "referee": fixture.get("referees", [{}])[0].get("name", "Unknown")
    ...
    def get_complete_season_data(self) -> Dict:
        ...
        return {
            ...
            "scheduled_fixtures": len(
                [f for f in all_fixtures if f.get("status") == "SCHEDULED"]
            ),
            "finished_fixtures": len(
                [f for f in all_fixtures if f.get("status") == "FINISHED"]
            ),
        }
