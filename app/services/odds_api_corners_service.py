        ...
        for bm in (ev.get('bookmakers') or []):
            ...
            for mk in (bm.get('markets') or []):
                ...
                for pt, pair in by_point.items():
                    over_odds = pair.get('over_odds'); under_odds = pair.get('under_odds')
...
def _parse_event_odds_history_payload(payload: Any, home: Optional[str], away: Optional[str], commence: Optional[str]) -> List[Dict[str, Any]]:
...
def fetch_corners_totals_from_odds_api(
    ...
    markets_try = markets or _env('ODDS_API_CORNERS_MARKETS', 'alternate_totals_corners')
    params = {
        'apiKey': api_key,
        'regions': regions,
        'markets': markets_try,
    ...
    if status == 422 and isinstance(payload, dict) and 'INVALID_MARKET' in str(payload).upper():
        for alt in ['alternate_totals_corners', 'totals_corners', 'total_corners', 'corners', 'alternate_total_corners']:
    ...
    if not rows:
        ...
        if historical or _env('ODDS_API_ALLOW_HISTORICAL', '0') == '1' or ts_from or ts_to:
            list_params_hist = {
                'apiKey': api_key,
                'regions': regions,
                'markets': 'h2h',
