...
class HistoricalEPLService:
    ...
    async def get_comprehensive_historical_data(self) -> Dict[str, SeasonData]:
        ...
        logger.info(f"Loading comprehensive historical data for seasons: {self.historical_seasons}")
        ...
        if season == "2025-26":
            ...
            try:
                if PREMIER_LEAGUE_AVAILABLE:
                    teams = get_teams()
                    if teams:
                        return [self._normalize_team_data(team, season) for team in teams]
            except Exception as e:
        ...
        if season == "2025-26":
            ...
            try:
                if PREMIER_LEAGUE_AVAILABLE:
                    ...
                    if current_matches:
                        matches = [self._normalize_match_data(match, season) for match in current_matches]
            except Exception as e:
        ...
        if season == "2025-26":
            ...
            try:
                if PREMIER_LEAGUE_AVAILABLE:
                    ...
                    if current_players:
                        players = [self._normalize_player_data(player, season) for player in current_players]
            except Exception as e:
    ...
    def _normalize_match_data(self, match_data: Dict, season: str) -> Dict:
        ...
        return {
            ...
            "result": self._determine_match_result(match_data.get("home_score", 0), match_data.get("away_score", 0)),
    ...
    def _get_standard_epl_teams(self, season: str) -> List[Dict]:
        ...
        standard_teams = [
            ...
            "Leicester City", "Ipswich Town"  # 2025-26 promoted teams
        ]
    ...
    async def _generate_season_players(self, season: str) -> List[Dict]:
        ...
        season_players = top_players_by_season.get(season, top_players_by_season["2025-26"])
    ...
    async def _calculate_season_table(self, teams_data: List[Dict], matches_data: List[Dict]) -> List[Dict]:
        ...
        sorted_table = sorted(table.values(), key=lambda x: (-x["points"], -x["goal_difference"]))
    ...
    async def _calculate_season_statistics(self, matches_data: List[Dict], players_data: List[Dict]) -> Dict:
    ...
    def get_ml_training_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"🤖 Prepared ML training data: {len(df)} matches across {len(dataset)} seasons")
