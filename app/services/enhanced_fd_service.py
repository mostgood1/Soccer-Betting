...
class EnhancedFDService:
    ...
    def refresh_data(self) -> Dict[str, Any]:
        try:
            ...
            return {
                "reloaded": True,
                "fixtures": len(
                    (self.football_data or {}).get("converted_fixtures", [])
                ),
            }
        except Exception as e:
    ...
    def get_matches(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for f in fixtures:
            ...
            home_score = m.get("home_score") or (
                ft.get("home") if isinstance(ft, dict) else None
            )
            away_score = m.get("away_score") or (
                ft.get("away") if isinstance(ft, dict) else None
            )
            status = str(m.get("status", "")).upper()
            is_completed = status in ("FINISHED", "COMPLETED") and (
    ...
    def get_system_status(self) -> Dict[str, Any]:
        try:
            return {
                ...
                "season": self.current_season,
                "total_fixtures": len(
                    (self.football_data or {}).get("converted_fixtures", []) or []
                ),
