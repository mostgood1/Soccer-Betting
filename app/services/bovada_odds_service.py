...
def _normalize_probs(values: Dict[str, Optional[float]]) -> Dict[str, float]:
    inv = {k: (v if isinstance(v, (int, float)) and v > 0 else 0.0) for k, v in values.items()}
    s = sum(inv.values())
    if s <= 0:
        ...
        def parse_event(ev: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            for dg in display_groups:
                ...
                for mk in markets:
                    ...
                    if any(k in desc for k in ('match result', '3-way moneyline', 'moneyline 3-way', 'result')) and len(outcomes) >= 2:
                        probs: Dict[str, Optional[float]] = {'H': None, 'D': None, 'A': None}
                        for o in outcomes:
                            oname = (o.get('description') or o.get('name') or '').lower()
                            price = o.get('price') or {}
                            american = price.get('american')
                            dec = _american_to_decimal(float(american)) if american is not None else None
                            imp = _implied_from_decimal(dec)
                            if 'draw' in oname:
                                probs['D'] = imp
                            elif home_name and _normalize_team(oname) == _normalize_team(home_name):
                                probs['H'] = imp
                            elif away_name and _normalize_team(oname) == _normalize_team(away_name):
                                probs['A'] = imp
                            elif 'home' in oname:
                                probs['H'] = imp
                            elif 'away' in oname:
                    ...
                    if ('total' in desc and 'half' not in desc) and len(outcomes) >= 2:
                        ...
                        for o in outcomes:
                            ...
                            hcap = o.get('price', {}).get('handicap') or mk.get('handicap') or o.get('handicap')
                            try:
                                if hcap is not None:
                                    line_val = float(hcap)
                            except Exception:
                            ...
                            if 'over' in oname:
                                over_imp = imp
                                over_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'under' in oname:
                                under_imp = imp
                                under_ml = int(american) if isinstance(american, (int, float)) else None
                        if over_imp is not None or under_imp is not None:
                            probs = _normalize_probs({'over': over_imp, 'under': under_imp})
                    ...
                    if ('both teams to score' in desc or 'btts' in desc) and len(outcomes) >= 2:
                        yes_imp = None; no_imp = None
                        yes_ml = None; no_ml = None
                        for o in outcomes:
                            oname = (o.get('description') or o.get('name') or '').lower()
                            price = o.get('price') or {}
                            american = price.get('american')
                            ...
                            imp = _implied_from_decimal(dec)
                            if 'yes' in oname:
                                yes_imp = imp; yes_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'no' in oname:
                                no_imp = imp; no_ml = int(american) if isinstance(american, (int, float)) else None
                        if yes_imp is not None or no_imp is not None:
                            btts = {'yes_prob': yes_imp, 'no_prob': no_imp, 'yes_ml': yes_ml, 'no_ml': no_ml}
                    ...
                    if ('double chance' in desc) and len(outcomes) >= 2:
                        ...
                        for o in outcomes:
                            ...
                            if 'home or draw' in oname or '1x' in oname:
                                ...
                                key = '12'
                            if key:
                                dc_map[key] = {'prob': imp, 'ml': int(american) if isinstance(american, (int, float)) else None}
                        if dc_map:
                            double_chance = {'1X': dc_map.get('1X'), 'X2': dc_map.get('X2'), '12': dc_map.get('12')}
                    # Draw No Bet (2-way)
                    if (('draw no bet' in desc) or ('moneyline' in desc and '2-way' in desc) or ('2 way' in desc)) and len(outcomes) >= 2:
                        dh_imp = None; da_imp = None
                        dh_ml = None; da_ml = None
                        for o in outcomes:
                            oname = (o.get('description') or o.get('name') or '').lower()
                            price = o.get('price') or {}
                            american = price.get('american')
                            ...
                            if home_name and name_norm == _normalize_team(home_name):
                                dh_imp = imp; dh_ml = int(american) if isinstance(american, (int, float)) else None
                            elif away_name and name_norm == _normalize_team(away_name):
                                da_imp = imp; da_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'home' in oname:
                                dh_imp = imp; dh_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'away' in oname:
                                da_imp = imp; da_ml = int(american) if isinstance(american, (int, float)) else None
                        if dh_imp is not None or da_imp is not None:
                            draw_no_bet = {'home_prob': dh_imp, 'away_prob': da_imp, 'home_ml': dh_ml, 'away_ml': da_ml}
                    # Total Corners (Full game)
                    if ('corner' in desc and 'total' in desc and 'team' not in desc) and len(outcomes) >= 2:
                        c_over_imp = None; c_under_imp = None
                        c_over_ml = None; c_under_ml = None
                        ...
                        for o in outcomes:
                            ...
                            try:
                                ...
                                pass
                            if 'over' in oname:
                                c_over_imp = imp; c_over_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'under' in oname:
                                c_under_imp = imp; c_under_ml = int(american) if isinstance(american, (int, float)) else None
                        if c_over_imp is not None or c_under_imp is not None:
                            probs = _normalize_probs({'over': c_over_imp, 'under': c_under_imp})
                    ...
                    if (('card' in desc or 'booking' in desc) and 'total' in desc and 'team' not in desc) and len(outcomes) >= 2:
                        k_over_imp = None; k_under_imp = None
                        k_over_ml = None; k_under_ml = None
                        ...
                        for o in outcomes:
                            ...
                            try:
                                ...
                                pass
                            if 'over' in oname:
                                k_over_imp = imp; k_over_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'under' in oname:
                                k_under_imp = imp; k_under_ml = int(american) if isinstance(american, (int, float)) else None
                        if k_over_imp is not None or k_under_imp is not None:
                            probs = _normalize_probs({'over': k_over_imp, 'under': k_under_imp})
                            cards_totals.append({
                                'line': k_line_val,
                                'over_prob': probs.get('over'),
                                'under_prob': probs.get('under'),
                    ...
                    if (('handicap' in desc or 'asian' in desc) and 'corner' not in desc) and len(outcomes) >= 2:
                        home_ml = None; away_ml = None
                        h_line: Optional[float] = None
                        h_prob: Optional[float] = None; a_prob: Optional[float] = None
                        for o in outcomes:
                            oname = (o.get('description') or o.get('name') or '').lower()
                            price = o.get('price') or {}
                            american = price.get('american')
                            ...
                            try:
                                ...
                                pass
                            name_norm = _normalize_team(oname)
                            if home_name and name_norm == _normalize_team(home_name) or 'home' in oname:
                                h_prob = imp; home_ml = int(american) if isinstance(american, (int, float)) else None
                                if ln is not None: h_line = ln
                            elif away_name and name_norm == _normalize_team(away_name) or 'away' in oname:
                                a_prob = imp; away_ml = int(american) if isinstance(american, (int, float)) else None
                                if ln is not None: h_line = ln if h_line is None else h_line
                    ...
                    if ('corner' in desc and 'handicap' in desc) and len(outcomes) >= 2:
                        ...
                        for o in outcomes:
                            ...
                            if home_name and name_norm == _normalize_team(home_name) or 'home' in oname:
                                h_prob = imp; home_ml = int(american) if isinstance(american, (int, float)) else None
                                if ln is not None: c_line = ln
                            elif away_name and name_norm == _normalize_team(away_name) or 'away' in oname:
                                a_prob = imp; away_ml = int(american) if isinstance(american, (int, float)) else None
                                if ln is not None: c_line = ln if c_line is None else c_line
                        if h_prob is not None or a_prob is not None:
                    ...
                    # First Half Totals
                    if any(k in desc for k in ('first half total', '1st half total', '1st half - total')) and len(outcomes) >= 2:
                        over_imp = None; under_imp = None
                        over_ml = None; under_ml = None
                        line_val: Optional[float] = None
                        for o in outcomes:
                            ...
                            try:
                                ...
                                pass
                            if 'over' in oname:
                                over_imp = imp; over_ml = int(american) if isinstance(american, (int, float)) else None
                            elif 'under' in oname:
                                under_imp = imp; under_ml = int(american) if isinstance(american, (int, float)) else None
                        if over_imp is not None or under_imp is not None:
                            probs = _normalize_probs({'over': over_imp, 'under': under_imp})
                            fh_totals.append({
                                'line': line_val,
                    ...
                    # Second Half Totals
                    if any(k in desc for k in ('second half total', '2nd half total', '2nd half - total')) and len(outcomes) >= 2:
                        over_imp = None; under_imp = None
                        over_ml = None; under_ml = None
                        line_val: Optional[float] = None
                        for o in outcomes:
                    ...
                    if ('team total' in desc or 'team goals' in desc) and len(outcomes) >= 2:
                    ...
                    if ('corner' in desc and 'team' in desc and 'total' in desc) and len(outcomes) >= 2:
