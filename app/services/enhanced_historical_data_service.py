...
class EnhancedHistoricalDataService:
    ...
    def __init__(self):
        ...
        print(
            f"Enhanced Historical Data Service: {len(self.available_seasons)} seasons loaded"
        )
    ...
    def _generate_season_data(self, season: str) -> Dict:
        ...
        if season == "2022-23":
            teams = [
                t
                for t in teams
                if t not in ["Luton Town", "Burnley", "Sheffield United"]
            ]
        ...
        for gameweek in range(1, 39):  # 38 gameweeks
            ...
            for i in range(0, len(teams_copy), 2):
                if i + 1 < len(teams_copy):
                    ...
                    match_result = self._generate_match_result(
            ...
            if len(gameweek_matches) != 10:
                logger.warning(
                    f"Gameweek {gameweek} has {len(gameweek_matches)} matches instead of 10"
                )

        return matches

    def _generate_match_result(
        ...
        home_expected_goals = (
            effective_home_strength * 1.4
        )  # Average 1.4 goals for neutral team at home
        away_expected_goals = (
            away_strength * 1.1
        )  # Average 1.1 goals for neutral team away
        ...
        shots_on_target_home = min(
            shots_home, max(home_goals, int(shots_home * np.random.uniform(0.25, 0.45)))
        )
        shots_on_target_away = min(
            shots_away, max(away_goals, int(shots_away * np.random.uniform(0.25, 0.45)))
        )
        ...
        possession_home = max(
            25,
            min(
                75, int(possession_base + possession_advantage + np.random.normal(0, 5))
            ),
        )
    ...
    def _generate_match_date(self, gameweek: int, season: str) -> str:
        ...
        if gameweek <= 15:
            ...
            start_date = datetime(season_start_year, 8, 15) + timedelta(
                weeks=gameweek - 1
            )
        else:
            # Jan-May of second year
            start_date = datetime(season_start_year + 1, 1, 15) + timedelta(
                weeks=gameweek - 16
            )
    ...
    def _generate_season_standings(self, teams: List[str], season: str) -> List[Dict]:
        ...
        for team in teams:
            ...
            draws = min(
                remaining, max(0, (points - wins * 3) + np.random.randint(-3, 4))
            )
    ...
    def _generate_season_player_stats(
        self, teams: List[str], season: str
    ) -> List[Dict]:
