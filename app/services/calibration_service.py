...
class CalibrationService:
    ...
    def _save(self):
        payload = {
            **self.meta,
            ...
            "by_league": self.temperatures_by_league,
        }
    ...
    def calibrate_up_to_week(
        self, up_to_week: int, league: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            if HIST_ODDS_PATH.exists():
                ...
                for rec in data.get("records", []):
                    ht = normalize_team_name(rec.get("home_team")) or rec.get(
                        "home_team"
                    )
                    at = normalize_team_name(rec.get("away_team")) or rec.get(
                        "away_team"
                    )
                    if not (ht and at):
                        continue
                    ...
                    if "bet365" in preferred_keys:
                        ...
                        if isinstance(b365, dict):
                            ...
                            if isinstance(ip, dict) and all(
                                k in ip for k in ("H", "D", "A")
                            ):
                                rec = {
                                    **rec,
                                    "preferred_implied": ip,
                                    "preferred_overround": ov
            ...
            for m in weeks.get(wk, []):
                ...
                try:
                    raw = advanced_ml_predictor.predict_match(
                        home, away, league=str(league).upper() if league else None
                    )
                    if not raw:
                        continue
                    p_home = float(
                        raw.get("home_win_probability") or raw.get("home_prob") or 0.34
                    )
                    p_away = float(
                        raw.get("away_win_probability") or raw.get("away_prob") or 0.33
                    )
                    p_draw = float(
                        raw.get("draw_probability") or max(0.0, 1 - (p_home + p_away))
                    )
                    ...
                    if mk:
                        cons = (
                            mk.get("preferred_implied")
                            or mk.get("consensus_implied")
                            or {}
                        )
                        mh = cons.get("H")
                        md = cons.get("D")
        ...
        if not X:
            self.temperature = None
            self.meta = {
                "trained_on": {"matches": 0, "weeks": list(range(1, up_to_week + 1))}
            }
            self._save()
        ...
        for T in [x / 20.0 for x in range(10, 61)]:  # 0.5 to 3.0 step 0.05
            ...
            if loss < best_loss:
                ...
                best_T = T
        # Fine-tune near best
        start = max(0.5, best_T - 0.2)
        end = min(3.0, best_T + 0.2)
        step = 0.01
        t = start
        while t <= end + 1e-9:
            loss = nll_for_T(t)
        ...
        return {
            "trained": True,
            "temperature": (
                self.temperatures_by_league.get(str(league).upper())
                if league
    ...
    def apply_if_ready(
        self, probs: Dict[str, float], league: Optional[str] = None
    ) -> Dict[str, float]:
...
__all__ = ["CalibrationService", "calibration_service", "apply_temperature"]
