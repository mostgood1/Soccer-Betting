...
class BettingOddsService:
    ...
    def clear_cache(self) -> Dict[str, int]:
        sizes = {
            ...
            'bovada_events_total': sum(len((snap or {}).get('events', [])) for snap in self._bovada_cache.values()) if isinstance(self._bovada_cache, dict) else 0,
        }
        self.cache.clear()
    ...
    def prefetch(self, sport_key: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self.odds_api_key:
            return {'provider': 'the-odds-api', 'prefetched': 0, 'note': 'ODDS_API_KEY not set'}
        targets = [sport_key] if sport_key else list(self.SPORT_KEYS)
        now = datetime.now()
        ok = 0; errs = []
        for s in targets:
            try:
                snap = fetch_the_odds_api(sport_key=s, markets='h2h', odds_format='decimal', api_key=self.odds_api_key)
                if isinstance(snap, dict) and not snap.get('error'):
                    self._sport_cache[s] = snap
                    self._sport_cache_expiry[s] = now + timedelta(seconds=self.cache_duration)
                    ok += 1
                else:
                    errs.append({s: snap.get('error') if isinstance(snap, dict) else 'unknown error'})
        ...
        def _do(key: str, fetcher):
            try:
                ...
                if isinstance(snap, dict) and 'events' in snap:
                    ...
                    self._bovada_cache_expiry[key] = now + timedelta(seconds=self.cache_duration)
    ...
    def _lookup_the_odds_api(self, home: str, away: str) -> Optional[Dict[str, Any]]:
        ...
        for sport in self.SPORT_KEYS:
            ...
            if not fresh or not exp or now >= exp:
                try:
                    snap = fetch_the_odds_api(sport_key=sport, markets='h2h', odds_format='decimal', api_key=self.odds_api_key)
                    if isinstance(snap, dict) and not snap.get('error'):
                        self._sport_cache[sport] = snap
                        self._sport_cache_expiry[sport] = now + timedelta(seconds=self.cache_duration)
                except Exception:
                    continue
            ...
            for rec in snap['records']:
                th = normalize_team_name(rec.get('home_team') or '') or (rec.get('home_team') or '')
                ta = normalize_team_name(rec.get('away_team') or '') or (rec.get('away_team') or '')
                if not th or not ta:
                    continue
                if th.lower() == home.lower() and ta.lower() == away.lower():
                    ...
                    bm = next((b for b in rec.get('bookmakers') or [] if any((m.get('outcomes') for m in b.get('markets') or []))), None)
                    if not bm:
                        continue
                    mk = next((m for m in bm.get('markets') or [] if m.get('key') == 'h2h' and m.get('outcomes')), None)
                    if not mk:
                        continue
                    outcomes = mk.get('outcomes') or []
                    # Expected names: 'HOME', 'DRAW', 'AWAY' (case may vary)
                    prices: Dict[str, Optional[float]] = {'H': None, 'D': None, 'A': None}
                    for o in outcomes:
                    ...
                    probs = self._normalize_market_odds({'home_odds': None, 'draw_odds': None, 'away_odds': None}, override_implied=prices)
                    ...
                    def _first_decimal(label: str) -> Optional[float]:
                        ...
                        for o in outcomes:
                            ...
                            if label == 'away' and 'AWAY' in name:
                                return dec
                        return None
                    dh = _first_decimal('home'); dd = _first_decimal('draw'); da = _first_decimal('away')
                    return {
                        'provider': 'the-odds-api',
                        'timestamp': datetime.now().isoformat(),
                        'market_odds': {
                            'match_winner': {
                                'home': {'team': rec.get('home_team'), 'odds': dh, 'odds_american': self._decimal_to_american(dh) if dh else None, 'probability': probs.get('H')},
                                'draw': {'odds': dd, 'odds_american': self._decimal_to_american(dd) if dd else None, 'probability': probs.get('D')},
                                'away': {'team': rec.get('away_team'), 'odds': da, 'odds_american': self._decimal_to_american(da) if da else None, 'probability': probs.get('A')},
                            }
                        },
                        'overround': None
                    }
    ...
    def _lookup_bovada_pl(self, home: str, away: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            for ev in snap['events']:
                eh = normalize_team_name(ev.get('home_team') or '') or (ev.get('home_team') or '')
                ea = normalize_team_name(ev.get('away_team') or '') or (ev.get('away_team') or '')
                if eh and ea and eh.lower() == home.lower() and ea.lower() == away.lower():
    ...
    def _identify_best_value(self, home_odds: float, draw_odds: float, away_odds: float) -> Dict:
    ...
    def get_betting_insights(self, fixtures_with_odds: List[Dict]) -> Dict:
        ...
        return {
            ...
            'outcome_percentages': {
                'home_wins': round((home_wins_expected / total_matches) * 100, 1) if total_matches > 0 else 0,
                'draws': round((draws_expected / total_matches) * 100, 1) if total_matches > 0 else 0,
                'away_wins': round((away_wins_expected / total_matches) * 100, 1) if total_matches > 0 else 0
            },
            'high_value_bets': [],
            'average_home_odds': self._calculate_average_odds('home', fixtures_with_odds),
            'average_draw_odds': self._calculate_average_odds('draw', fixtures_with_odds),
            'average_away_odds': self._calculate_average_odds('away', fixtures_with_odds)
        }
    ...
    def _build_from_bovada(self, ev: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(v is not None for v in (dh, dd, da)):
            out['market_odds']['match_winner'] = {
                'home': {'team': ev.get('home_team'), 'odds': dh, 'odds_american': self._decimal_to_american(dh) if dh else None, 'probability': h2h.get('H')},
                'draw': {'odds': dd, 'odds_american': self._decimal_to_american(dd) if dd else None, 'probability': h2h.get('D')},
                'away': {'team': ev.get('away_team'), 'odds': da, 'odds_american': self._decimal_to_american(da) if da else None, 'probability': h2h.get('A')},
            }
        # Add non-H2H markets
        self._inject_bovada_markets(out, ev)
        return out

    def _merge_bovada_non_h2h(self, odds: Dict[str, Any], ev: Dict[str, Any]) -> Dict[str, Any]:
        if not odds:
            return self._build_from_bovada(ev)
    ...
    def _inject_bovada_markets(self, base: Dict[str, Any], ev: Dict[str, Any]) -> None:
        ...
        if totals:
            rows = []
            for t in totals:
                over_prob = t.get('over_prob'); under_prob = t.get('under_prob')
                over_dec = self._prob_to_decimal(over_prob); under_dec = self._prob_to_decimal(under_prob)
                rows.append({
                    'line': t.get('line'),
                    'over': {'probability': over_prob, 'odds': over_dec, 'odds_american': self._decimal_to_american(over_dec) if over_dec else None},
                    'under': {'probability': under_prob, 'odds': under_dec, 'odds_american': self._decimal_to_american(under_dec) if under_dec else None},
                })
            mo['totals'] = rows
        # First half totals
        ...
        if btts:
            mo['both_teams_to_score'] = {
                'yes': {
                    ...
                    'odds_american': self._decimal_to_american(self._prob_to_decimal(btts.get('yes_prob'))) if self._prob_to_decimal(btts.get('yes_prob')) else None
                },
                'no': {
                    'probability': btts.get('no_prob'),
                    'odds': self._prob_to_decimal(btts.get('no_prob')),
                    'odds_american': self._decimal_to_american(self._prob_to_decimal(btts.get('no_prob'))) if self._prob_to_decimal(btts.get('no_prob')) else None
                }
            }
        # Double Chance
        ...
        if dc:
            def mk(prob):
                ...
                return {'probability': prob, 'odds': dec, 'odds_american': self._decimal_to_american(dec) if dec else None}
            mo['double_chance'] = {
            ...
            for t in ah:
                rows.append({
                    ...
                    'home': {
                        ...
                        'odds_american': self._decimal_to_american(self._prob_to_decimal(t.get('home_prob'))) if self._prob_to_decimal(t.get('home_prob')) else None
                    },
                    'away': {
                        'probability': t.get('away_prob'),
                        'odds': self._prob_to_decimal(t.get('away_prob')),
                        'odds_american': self._decimal_to_american(self._prob_to_decimal(t.get('away_prob'))) if self._prob_to_decimal(t.get('away_prob')) else None
                    }
                })
            ...
            for t in cards:
                rows.append({
                    ...
                    'over': {
                        ...
                        'odds_american': self._decimal_to_american(self._prob_to_decimal(t.get('over_prob'))) if self._prob_to_decimal(t.get('over_prob')) else None
                    },
                    'under': {
                        'probability': t.get('under_prob'),
                        'odds': self._prob_to_decimal(t.get('under_prob')),
                        'odds_american': self._decimal_to_american(self._prob_to_decimal(t.get('under_prob'))) if self._prob_to_decimal(t.get('under_prob')) else None
                    }
                })
            mo['cards_totals'] = rows
    def _normalize_market_odds(self, normalized_input: Dict[str, Any], override_implied: Optional[Dict[str, Optional[float]]] = None) -> Dict[str, float]:
        ...
        if override_implied is not None:
            inv = {k: (v if isinstance(v, (int, float)) and v > 0 else 0.0) for k, v in override_implied.items()}
            s = sum(inv.values())
            if s <= 0:
                return {'H': 0.0, 'D': 0.0, 'A': 0.0}
            return {k: inv[k] / s for k in ('H', 'D', 'A')}
        try:
            h = float(normalized_input.get('home_odds')) if normalized_input.get('home_odds') else None
            d = float(normalized_input.get('draw_odds')) if normalized_input.get('draw_odds') else None
            a = float(normalized_input.get('away_odds')) if normalized_input.get('away_odds') else None
            inv = {'H': (1.0/h if h and h > 1.0 else 0.0), 'D': (1.0/d if d and d > 1.0 else 0.0), 'A': (1.0/a if a and a > 1.0 else 0.0)}
            s = sum(inv.values())
            if s <= 0:
                return {'H': 0.0, 'D': 0.0, 'A': 0.0}
            return {k: inv[k] / s for k in ('H', 'D', 'A')}
    ...
    def _calculate_average_odds(self, outcome: str, fixtures_with_odds: List[Dict]) -> float:
