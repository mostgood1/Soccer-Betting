...
from ..services.league_manager import get_service as get_league_service, normalize_league_code
...
def daily_update(
    ...
    odds_api_bookmakers: Optional[str] = 'pinnacle,bet365,williamhill,unibet,betfair_ex',
) -> Dict[str, Any]:
    ...
    try:
        ...
        step('week_detection', {'current_week': current_week, 'previous_week': prev_week, 'weeks_found': len(weeks_map)})
    except Exception as e:
        current_week = 1; prev_week = 1
    ...
    for w in [prev_week, current_week]:
        ...
        try:
            res = fetch_scores('PL', 2025, w, auto_reconcile=True, force_repredict=False)
    ...
    if include_odds_api_corners and os.getenv('ODDS_API_KEY'):
        try:
            from ..services.odds_api_corners_service import fetch_corners_totals_from_odds_api as _fetch_corners
                ...
                try:
                    ...
                    for r in new_recs:
                        try:
                            key = _k(r)
                            if key not in seen:
                                merged.append(r); seen.add(key); added_total += 1
                        except Exception:
                            merged.append(r); added_total += 1
                except Exception:
                    # Continue with other leagues
                    continue
            out_payload = {'records': merged, 'source': 'oddsapi', 'added_now': added_total}
            out_path.write_text(json.dumps(out_payload, indent=2), encoding='utf-8')
            totals, team = reload_market_store()
            step('update_corners_oddsapi', {'added_now': added_total, 'total_records': len(merged), 'reloaded': {'totals': totals, 'team': team}})
        except Exception as e:
            summary['errors'].append({'update_corners_oddsapi': str(e)})
    else:
        step('update_corners_oddsapi', {'skipped': True, 'reason': 'no ODDS_API_KEY or disabled'})
    ...
    if include_bovada_corners:
        try:
            ...
            from ..services.bovada_odds_service import fetch_pl_odds as _fetch_bov, _american_to_decimal as _ml_to_dec
            ...
            for ev in events:
                ...
                for t in (ev.get('corners_totals') or []):
                    ...
                    if not home or not away or not date or line is None:
                        continue
                    op = t.get('over_prob'); up = t.get('under_prob')
                    oml = t.get('over_ml'); uml = t.get('under_ml')
                    over_odds = _ml_to_dec(float(oml)) if oml is not None else (1.0/float(op) if isinstance(op,(int,float)) and float(op)>0 else None)
                    under_odds = _ml_to_dec(float(uml)) if uml is not None else (1.0/float(up) if isinstance(up,(int,float)) and float(up)>0 else None)
                    if over_odds and under_odds:
                        rows.append({'date': date,'home_team': home,'away_team': away,'line': float(line),'over_odds': float(round(over_odds,4)),'under_odds': float(round(under_odds,4)),'bookmaker': 'Bovada','market_type': 'total'})
                for t in (ev.get('team_corners') or []):
                    ...
                    under_odds = _ml_to_dec(float(uml)) if uml is not None else (1.0/float(up) if isinstance(up,(int,float)) and float(up)>0 else None)
                    if over_odds and under_odds:
                        rows.append({'date': date,'home_team': home,'away_team': away,'side': side,'line': float(line),'over_odds': float(round(over_odds,4)),'under_odds': float(round(under_odds,4)),'bookmaker': 'Bovada','market_type': 'team'})
            out_path = Path('data/corners_market_lines_manual.json')
            existing: List[Dict[str, Any]] = []
            try:
                if out_path.exists():
            ...
            for r in rows:
                try:
                    key = _k2(r)
                    if key not in seen:
                        merged.append(r); seen.add(key); added_now += 1
                except Exception:
                    merged.append(r); added_now += 1
            out_path.write_text(json.dumps({'records': merged, 'source': 'bovada', 'added_now': added_now}, indent=2), encoding='utf-8')
            totals, team = reload_market_store()
            step('bovada_corners_backfill', {'added_now': added_now, 'total_records': len(merged), 'reloaded': {'totals': totals, 'team': team}})
        except Exception as e:
    ...
    if capture_closing:
        try:
            ...
            step('capture_closing', {'week': current_week, 'status': closing.get('status')})
        except Exception as e:
    ...
    if generate_snapshot:
        try:
            ...
            payload['markets']['match_odds'] = compare_week_odds(current_week, edge_threshold=edge)
            payload['markets']['totals_full'] = compare_week_totals(current_week, line=2.5, edge_threshold=edge)
            payload['markets']['totals_first_half'] = compare_week_first_half_totals(current_week, line=1.0, edge_threshold=edge)
            payload['markets']['totals_second_half'] = compare_week_second_half_totals(current_week, line=1.0, edge_threshold=edge)
            payload['markets']['corners_totals'] = compare_week_corners_totals(current_week, line=9.5, edge_threshold=edge)
            payload['markets']['team_goals_home'] = compare_week_team_goals_totals(current_week, side='home', line=1.5, edge_threshold=edge)
            payload['markets']['team_goals_away'] = compare_week_team_goals_totals(current_week, side='away', line=1.5, edge_threshold=edge)
            payload['markets']['team_corners_home'] = compare_week_team_corners_totals(current_week, side='home', line=4.5, edge_threshold=edge)
            payload['markets']['team_corners_away'] = compare_week_team_corners_totals(current_week, side='away', line=4.5, edge_threshold=edge)
            ...
            step('odds_snapshot', {'saved': str(out_path), 'markets': list(payload['markets'].keys())})
        except Exception as e:
...
def _normalize_prediction(raw: Dict[str, Any], home_team: str, away_team: str) -> Dict[str, Any]:
...
def reconcile_week(week: int, manual_path: str | None = None, force_repredict: bool = False, apply_goal_adjust: bool = False) -> Dict[str, Any]:
    ...
    if apply_goal_adjust:
        ...
        existing = [r for r in reconciliation_store.dump().get('reconciliations', {}).values() if r.get('status')=='completed' and r.get('game_week')!=week]
        if existing:
            sh = [r['predicted']['home_goals'] - r['actual']['home_goals'] for r in existing]
            sa = [r['predicted']['away_goals'] - r['actual']['away_goals'] for r in existing]
    ...
    for m in wk_matches:
        ...
        if apply_goal_adjust and offsets:
            ...
            rec = reconciliation_service.reconcile_match_predictions(match_record, reconcile_input)
...
def fetch_scores(competition: str, season: int, week: int, auto_reconcile: bool = False, force_repredict: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        for m in matches:
            ...
            if home_score is None or away_score is None or status not in ('FINISHED', 'IN_PLAY', 'TIMED'): # FINISHED primary, but keep others if partial
                continue
            manual_rows.append({
                'match_id': m.get('id'),
        ...
        if auto_reconcile and manual_rows:
            rec = reconcile_week(week, manual_path=str(manual_path), force_repredict=force_repredict)
            result['reconciliation'] = rec
        return result
    except Exception as e:
        return {'error': str(e)}

def diagnostics(week: int | None = None, calibration: bool = False, goal_adjust: bool = False) -> Dict[str, Any]:
    ...
    for r in completed:
        ...
        if calibration:
            phw, pdw, paw = p.get('home_win_prob'), p.get('draw_prob'), p.get('away_win_prob')
            if None not in (phw, pdw, paw):
                max_prob = max(phw, pdw, paw)
                pred_cls = 'H' if phw >= pdw and phw >= paw else ('D' if pdw >= paw else 'A')
                actual_cls = a.get('result') or ('H' if ah > aa else 'A' if aa > ah else 'D')
                calibration_rows.append({
                    'p': max_prob,
                    'correct': int(pred_cls == actual_cls)
                })
    ...
    if calibration and calibration_rows:
        ...
        for lo in [i/10 for i in range(0,10)]:
            hi = lo + 0.1
            bucket = [r for r in calibration_rows if lo <= r['p'] < hi or (hi >=1.0 and r['p']==1.0)]
            if not bucket:
                continue
...
# ---------------------------------------------------------------------------
def walkforward_train(start_week: int, end_week: int, fetch_scores_first: bool = True, retrain_level: str = 'patch', apply_goal_adjust: bool = False) -> Dict[str, Any]:
    ...
    for w in range(start_week, end_week + 1):
        ...
        if fetch_scores_first:
            ...
            if p.exists():
                manual_path = str(p)
        rec = reconcile_week(w, manual_path=manual_path, force_repredict=False, apply_goal_adjust=apply_goal_adjust)
        step['reconcile'] = rec
        # 2) Calibrate up to this week (uses completed matches only)
        cal = calibration_service.calibrate_up_to_week(w)
        step['calibration'] = { 'up_to_week': w, 'result': cal, 'status': calibration_service.status() }
        # 3) Retrain & bump model version
        ver = advanced_ml_predictor.retrain_and_bump(level=retrain_level)
        model_versions.append(ver)
        step['retrain'] = { 'model_version': ver, 'performance': advanced_ml_predictor.get_model_performance() }
...
# ---------------------------------------------------------------------------
def _load_historic_odds_index(cache_path: str = 'cache/historic_odds.json') -> Dict[str, Dict[str, Any]]:
    ...
    preferred_keys: Set[str] = {k.strip().lower() for k in pref_env.split(',') if k.strip()}
    try:
        p = Path(cache_path)
        if not p.exists():
            return idx
        ...
        for rec in data.get('records', []):
            ...
            if preferred_implied is not None:
                rec = {**rec, 'preferred_implied': preferred_implied, 'preferred_overround': preferred_over}
...
def compare_week_odds(week: int, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    ...
    matches = service.get_all_matches() if hasattr(service, 'get_all_matches') else EnhancedEPLService().get_all_matches()
    weeks = game_week_service.organize_matches_by_week(matches)
    wk_matches = weeks.get(week, [])
    ...
    try:
        if fetch_h2h_odds is not None:
            ...
            live_payload = fetch_h2h_odds(sport_key=sport_key, regions=regions, markets='h2h')
            ...
            for ev in events:
                ...
                def has_probs(bm: Dict[str, Any]) -> bool:
                    ...
                    return all(isinstance(ip.get(k), (int,float)) and ip.get(k) > 0 for k in ('H','D','A'))
                preferred_present = [bm for bm in bms if ((bm.get('key') or '').lower() in preferred_keys or (bm.get('title') or '').lower() in preferred_keys)]
                ...
                if all(consensus.get(k) is not None for k in ('H','D','A')):
                    ...
                    if s and s > 0:
                        consensus = {k: float(consensus[k])/s for k in ('H','D','A')}
                ...
                if any(vs for vs in probs_acc.values()):
                    live_idx[key_basic] = {
                        ...
                        'source': ('live_preferred' if filtered_valid else 'live_odds_api'),
    ...
    for m in wk_matches:
        ...
        if market_rec:
            ...
            cons = (market_rec.get('preferred_implied') or market_rec.get('consensus_implied') or {})
            h = cons.get('H'); d = cons.get('D'); a = cons.get('A')
            ...
            overround = market_rec.get('preferred_overround') if market_rec.get('preferred_implied') else market_rec.get('consensus_overround')
            market_source = 'historic_bet365' if market_rec.get('preferred_implied') else 'historic_consensus'
        ...
        if pred:
            model_probs = {
                'H': pred.get('home_win_prob'), 'D': pred.get('draw_prob'), 'A': pred.get('away_win_prob')
            }
            ...
            model_pick = max(model_probs.items(), key=lambda kv: (kv[1] if isinstance(kv[1], (int,float)) else -1))[0]
        market_pick = None
        if market_probs:
        ...
        if model_probs and market_probs and model_pick:
            edge_for_model_pick = round(model_probs[model_pick] - market_probs.get(model_pick, 0.0),4)
            edge_recommendation = edge_for_model_pick is not None and edge_for_model_pick >= edge_threshold
        ...
        if actual_result and model_probs and market_probs:
            ...
            for probs, bucket in ((model_probs, model_brier), (market_probs, market_brier)):
                b = 0.0
                for k in ('H','D','A'):
        ...
        rows.append({
            ...
            'model_probs': {k: (round(v,4) if isinstance(v,(int,float)) else None) for k,v in (model_probs or {}).items()} if model_probs else None,
            'market_probs': {k: (round(v,4) if isinstance(v,(int,float)) else None) for k,v in (market_probs or {}).items()} if market_probs else None,
    ...
    pick_alignments = sum(1 for r in rows if r.get('model_pick') and r.get('market_pick') and r['model_pick']==r['market_pick'])
    ...
    agg = {
        ...
        'model_accuracy': (model_correct/total_evaluable) if total_evaluable else None,
        'market_accuracy': (market_correct/total_evaluable) if total_evaluable else None,
        'model_log_loss': (sum(model_log_losses)/len(model_log_losses)) if model_log_losses else None,
        'market_log_loss': (sum(market_log_losses)/len(market_log_losses)) if market_log_losses else None,
        'model_brier': (sum(model_brier)/len(model_brier)) if model_brier else None,
        'market_brier': (sum(market_brier)/len(market_brier)) if market_brier else None,
    ...
    if agg['model_log_loss'] is not None and agg['market_log_loss'] is not None:
        agg['model_log_loss_delta'] = round(agg['market_log_loss'] - agg['model_log_loss'],4)
...
# ---------------------------------------------------------------------------
def compare_week_totals(week: int, line: float = 2.5, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if g:
                ...
                return {
                    'line': used_line,
                    'over_implied': over_p,
                    'under_implied': (1.0 - over_p) if isinstance(over_p, (int,float)) else None,
                    'overround': None,
    ...
    def get_totals_from_historic(rec: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for bm_data in bms.values():
            ...
            if float(t.get('line', 2.5)) != float(line):
                continue
            oi = t.get('over_implied'); ui = t.get('under_implied'); oor = t.get('overround')
            if isinstance(oi,(int,float)): over_vals.append(float(oi))
            if isinstance(ui,(int,float)): under_vals.append(float(ui))
            if isinstance(oor,(int,float)): overrounds.append(float(oor))
        if over_vals and under_vals:
            try:
                ...
                return {
                    ...
                    'overround': float(stats.median(overrounds)) if overrounds else None
                }
            except Exception:
                return {
                    'line': line,
                    ...
                    'overround': sum(overrounds)/len(overrounds) if overrounds else None
                }
        return None
    ...
    for m in wk_matches:
        ...
        if (not market_totals) and date and code == 'PL':
            ...
            if by_match:
                ...
                if line in by_match:
                    ...
                    try:
                        ...
                        if candidates:
                            chosen_line = min(candidates, key=lambda L: abs(float(L) - float(line)))
            ...
            if chosen_line is not None:
                ...
                op = bt.get('over_prob'); up = bt.get('under_prob')
                if isinstance(op, (int,float)) or isinstance(up, (int,float)):
                    try:
                        s = (float(op) if isinstance(op,(int,float)) else 0.0) + (float(up) if isinstance(up,(int,float)) else 0.0)
                        over_p = (float(op)/s) if s>0 else float(op) if isinstance(op,(int,float)) else None
                    except Exception:
                        over_p = None
                    market_totals = {
        ...
        if market_totals and all(isinstance(market_totals.get(k),(int,float)) for k in ('over_implied','under_implied')):
            # normalize to sum to 1 even if rounding noise exists
            ov = float(market_totals['over_implied']); un = float(market_totals['under_implied'])
            s = ov + un
            if s > 0:
        ...
        if model_over_prob is not None and market_over_prob is not None and model_pick:
            if model_pick == 'Over':
                ...
                edge_for_model_pick = round((1.0 - model_over_prob) - (1.0 - market_over_prob), 4)
            edge_recommendation = edge_for_model_pick >= edge_threshold

        # Metrics (binary)
        if actual_over is not None and model_over_prob is not None and market_over_prob is not None:
            total_evaluable += 1
            import math
            ...
            if (model_pick == 'Over' and actual_over==1) or (model_pick == 'Under' and actual_over==0):
                model_correct += 1
            if (market_pick == 'Over' and actual_over==1) or (market_pick == 'Under' and actual_over==0):
                market_correct += 1
        ...
        rows.append({
            ...
            'model_over_prob': round(model_over_prob,4) if isinstance(model_over_prob,(int,float)) else None,
            'market_over_prob': round(market_over_prob,4) if isinstance(market_over_prob,(int,float)) else None,
            'market_line': (market_totals.get('line') if isinstance(market_totals, dict) else None),
            # Flag if a nearest-line fallback was used (e.g., requested 2.5 -> used 2.75)
            'market_line_fallback': (isinstance(market_totals, dict) and market_totals.get('line') is not None and isinstance(line, (int, float)) and float(market_totals.get('line')) != float(line)),
            'market_line_delta': (abs(float(market_totals.get('line')) - float(line)) if (isinstance(market_totals, dict) and market_totals.get('line') is not None and isinstance(line,(int,float))) else None),
            'model_pick': model_pick,
            'market_pick': market_pick,
            'edge_for_model_pick': edge_for_model_pick,
            'edge_recommendation': edge_recommendation,
    ...
    agg = {
        ...
        'model_log_loss': (sum(model_log_losses)/len(model_log_losses)) if model_log_losses else None,
        'market_log_loss': (sum(market_log_losses)/len(market_log_losses)) if market_log_losses else None,
        'model_brier': (sum(model_briers)/len(model_briers)) if model_briers else None,
...
# ---------------------------------------------------------------------------
def compare_week_first_half_totals(week: int, line: float = 1.0, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    if not _prediction_cache:
        rebuild_predictions()
    code = normalize_league_code(league)
    service = get_league_service(code)
    ...
    for m in wk_matches:
        ...
        try:
            ...
            if market_over_prob is None and (chosen_line is not None):
                ...
                if isinstance(op, (int,float)) or isinstance(up, (int,float)):
                    s = (float(op) if isinstance(op,(int,float)) else 0.0) + (float(up) if isinstance(up,(int,float)) else 0.0)
                    market_over_prob = (float(op)/s) if s>0 else (float(op) if isinstance(op,(int,float)) else None)
        ...
        model_pick = 'Over' if (model_over_prob is not None and model_over_prob >= 0.5) else ('Under' if model_over_prob is not None else None)
        market_pick = 'Over' if (market_over_prob is not None and market_over_prob >= 0.5) else ('Under' if market_over_prob is not None else None)

        # Compute edge for model pick when market available
        edge_for_model_pick = None
        edge_recommendation = False
        if model_pick and isinstance(model_over_prob, (int, float)) and isinstance(market_over_prob, (int, float)):
            if model_pick == 'Over':
                edge_for_model_pick = round(float(model_over_prob) - float(market_over_prob), 4)
            else:  # Under
                edge_for_model_pick = round((1.0 - float(model_over_prob)) - (1.0 - float(market_over_prob)), 4)
            edge_recommendation = (edge_for_model_pick is not None) and (edge_for_model_pick >= edge_threshold)
        ...
        rows.append({
            ...
            'model_pick': model_pick,
            'market_pick': market_pick,
            'edge_for_model_pick': None if (market_over_prob is None or model_over_prob is None or model_pick is None) else (round(model_over_prob - market_over_prob,4) if model_pick=='Over' else round((1.0-model_over_prob)-(1.0-market_over_prob),4)),
            'edge_recommendation': False if (market_over_prob is None) else ((round(model_over_prob - market_over_prob,4) if model_pick=='Over' else round((1.0-model_over_prob)-(1.0-market_over_prob),4)) >= edge_threshold if model_pick else False),
            'market_source': market_source,
            # Flag if a nearest-line fallback was used (requested line vs returned market_line)
            'market_line_fallback': (isinstance(market_line, (int, float)) and isinstance(line, (int, float)) and float(market_line) != float(line)) or False,
            'market_line_delta': (abs(float(market_line) - float(line)) if (isinstance(market_line,(int,float)) and isinstance(line,(int,float))) else None)
        })
    ...
    agg = {
        ...
        'evaluable_with_actual_and_probs': total_evaluable,
        'model_accuracy': (model_correct/total_evaluable) if total_evaluable else None,
        'market_accuracy': (market_correct/total_evaluable) if (total_evaluable and market_log_losses) else None,
        'model_log_loss': (sum(model_log_losses)/len(model_log_losses)) if model_log_losses else None,
        'market_log_loss': (sum(market_log_losses)/len(market_log_losses)) if market_log_losses else None,
        'model_brier': (sum(model_briers)/len(model_briers)) if model_briers else None,
        'market_brier': (sum(market_briers)/len(market_briers)) if market_briers else None,
...
# ---------------------------------------------------------------------------
def compare_week_second_half_totals(week: int, line: float = 1.0, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    if not _prediction_cache:
        rebuild_predictions()
    code = normalize_league_code(league)
    service = get_league_service(code)
...
# ---------------------------------------------------------------------------
def compare_week_corners_totals(week: int, line: float = 9.5, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    if not _prediction_cache:
        rebuild_predictions()
    code = normalize_league_code(league)
    service = get_league_service(code)
    ...
    for m in wk_matches:
        ...
        actual_corners = None
        if status_completed:
            if 'total_corners' in m and isinstance(m.get('total_corners'), (int, float)):
                ...
                if isinstance(hc, (int, float)) and isinstance(ac, (int, float)):
                    ...
                    if c_actual and isinstance(c_actual.get('total_corners'), (int, float)):
        ...
        if pred:
            try:
                ...
                model_over_prob = corners_calibration_service.predict_over_prob(mu, line, week=week, market_over_prob=market_over_prob)
        ...
        rows.append({
            ...
            'market_line_fallback': (isinstance(market_line,(int,float)) and isinstance(line,(int,float)) and float(market_line) != float(line)) or False,
            'market_line_delta': (abs(float(market_line) - float(line)) if (isinstance(market_line,(int,float)) and isinstance(line,(int,float))) else None),
            'calibration': corners_calibration_service.status() if rows == [] else None
        })
...
# ---------------------------------------------------------------------------
def compare_week_team_goals_totals(week: int, side: str = 'home', line: float = 1.5, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    s = side.lower(); assert s in ('home','away'), "side must be 'home' or 'away'"
    if not _prediction_cache:
        rebuild_predictions()
    code = normalize_league_code(league)
    service = get_league_service(code)
    ...
    for m in wk_matches:
        ...
        try:
            ...
            if market_over_prob is None and (chosen_line is not None):
                ...
                op = bt.get('over_prob'); up = bt.get('under_prob')
                if isinstance(op, (int,float)) or isinstance(up, (int,float)):
                    ssum = (float(op) if isinstance(op,(int,float)) else 0.0) + (float(up) if isinstance(up,(int,float)) else 0.0)
                    market_over_prob = (float(op)/ssum) if ssum>0 else (float(op) if isinstance(op,(int,float)) else None)
                    market_source = 'live_bovada'
                    market_line = bt.get('market_line')
        except Exception:
            pass
        ...
        rows.append({
            ...
            'actual_over': (1 if (isinstance(actual_team_goals, (int, float)) and isinstance(line, (int, float)) and float(actual_team_goals) > float(line)) else (0 if (isinstance(actual_team_goals,(int,float)) and isinstance(line,(int,float))) else None)),
            'line': line,
            'market_line': market_line,
            'model_over_prob': round(model_over_prob,4) if isinstance(model_over_prob,(int,float)) else None,
            'market_over_prob': round(market_over_prob,4) if isinstance(market_over_prob,(int,float)) else None,
...
# ---------------------------------------------------------------------------
def compare_week_team_corners_totals(week: int, side: str = 'home', line: float = 4.5, edge_threshold: float = 0.05, league: Optional[str] = None) -> Dict[str, Any]:
    s = side.lower(); assert s in ('home','away'), "side must be 'home' or 'away'"
    if not _prediction_cache:
        rebuild_predictions()
    code = normalize_league_code(league)
    ...
    for m in wk_matches:
        ...
        rows.append({
            ...
            'actual_over': (1 if (isinstance(actual_team_corners, (int, float)) and isinstance(line, (int, float)) and float(actual_team_corners) > float(line)) else (0 if (isinstance(actual_team_corners,(int,float)) and isinstance(line,(int,float))) else None)),
            'line': line,
            'market_line': market_line,
            'model_over_prob': round(model_over_prob,4) if isinstance(model_over_prob,(int,float)) else None,
            'market_over_prob': round(market_over_prob,4) if isinstance(market_over_prob,(int,float)) else None,
...
# ---------------------------------------------------------------------------
def compare_week_double_chance(week: int, market: str = '1X', edge_threshold: float = 0.05) -> Dict[str, Any]:
    ...
    for m in wk_matches:
        ...
        if market_rec:
            ...
            if all(isinstance(v,(int,float)) for v in (h,d,a)):
                s = h + d + a
                market_probs = {'H': h/s, 'D': d/s, 'A': a/s} if s>0 else {'H': h, 'D': d, 'A': a}
                market_source = 'historic_bet365' if market_rec.get('preferred_implied') else 'historic_consensus'
        model_probs = None
        if pred:
            mp = {'H': pred['home_win_prob'],'D': pred['draw_prob'],'A': pred['away_win_prob']}
        ...
        # Edge & metrics
        model_pick = 'Yes' if (model_dc is not None and model_dc >= 0.5) else 'No' if model_dc is not None else None
        market_pick = 'Yes' if (market_dc is not None and market_dc >= 0.5) else 'No' if market_dc is not None else None
        edge_for_model_pick = None
        edge_recommendation = False
        if model_dc is not None and market_dc is not None and model_pick:
            edge_for_model_pick = round((model_dc - market_dc) if model_pick=='Yes' else ((1-model_dc)-(1-market_dc)),4)
            edge_recommendation = edge_for_model_pick >= edge_threshold
        if actual_result and model_dc is not None and market_dc is not None:
            total_evaluable += 1
            actual_yes = 1 if ((mkt=='1X' and actual_result in ('H','D')) or (mkt=='12' and actual_result in ('H','A')) or (mkt=='X2' and actual_result in ('D','A'))) else 0
            import math
            ...
            if (model_pick=='Yes' and actual_yes==1) or (model_pick=='No' and actual_yes==0):
                model_correct += 1
            if (market_pick=='Yes' and actual_yes==1) or (market_pick=='No' and actual_yes==0):
                market_correct += 1
        rows.append({
            'match_id': mid,
            'home_team': home_raw,
            ...
            'actual_result': actual_result,
            'market': mkt,
            'model_dc_prob': round(model_dc,4) if isinstance(model_dc,(int,float)) else None,
            'market_dc_prob': round(market_dc,4) if isinstance(market_dc,(int,float)) else None,
            'model_pick': model_pick,
            'market_pick': market_pick,
            'edge_for_model_pick': edge_for_model_pick,
            'edge_recommendation': edge_recommendation,
...
# ---------------------------------------------------------------------------
def compare_week_dnb(week: int, side: str = 'home', edge_threshold: float = 0.05) -> Dict[str, Any]:
    ...
    for m in wk_matches:
        ...
        if market_rec:
            ...
            if all(isinstance(v,(int,float)) for v in (h,d,a)):
                ssum = h + d + a
                market_probs = {'H': h/ssum, 'D': d/ssum, 'A': a/ssum} if ssum>0 else {'H': h, 'D': d, 'A': a}
                market_source = 'historic_bet365' if market_rec.get('preferred_implied') else 'historic_consensus'
        model_probs = None
        if pred:
            mp = {'H': pred['home_win_prob'],'D': pred['draw_prob'],'A': pred['away_win_prob']}
        ...
        if model_dnb is not None:
            model_pick = ('Home DNB' if s=='home' else 'Away DNB') if model_dnb >= 0.5 else ('Opp DNB')
        if market_dnb is not None:
            market_pick = ('Home DNB' if s=='home' else 'Away DNB') if market_dnb >= 0.5 else ('Opp DNB')
        edge_for_model_pick = None
        edge_recommendation = False
        if model_dnb is not None and market_dnb is not None and model_pick:
            if (s=='home' and model_pick=='Home DNB') or (s=='away' and model_pick=='Away DNB'):
        ...
        if actual_result in ('H','A') and model_dnb is not None and market_dnb is not None:
            total_evaluable += 1
            actual_yes = 1 if ((s=='home' and actual_result=='H') or (s=='away' and actual_result=='A')) else 0
            import math
            ...
            if ((s=='home' and model_pick=='Home DNB') or (s=='away' and model_pick=='Away DNB')) == (actual_yes==1):
                model_correct += 1
            if ((s=='home' and market_pick=='Home DNB') or (s=='away' and market_pick=='Away DNB')) == (actual_yes==1):
                market_correct += 1
        rows.append({
            'match_id': mid,
            'home_team': home_raw,
            ...
            'actual_result': actual_result,
            'side': s,
            'model_dnb_prob': round(model_dnb,4) if isinstance(model_dnb,(int,float)) else None,
            'market_dnb_prob': round(market_dnb,4) if isinstance(market_dnb,(int,float)) else None,
            'model_pick': model_pick,
            'market_pick': market_pick,
            'edge_for_model_pick': edge_for_model_pick,
            'edge_recommendation': edge_recommendation,
...
# ---------------------------------------------------------------------------
def _compute_consensus_and_edges(odds_payload: Dict[str, Any], edge_threshold: float = 0.05) -> Dict[str, Any]:
    ...
    for ev in events:
        ...
        if any(probs_acc[k] for k in probs_acc):
            consensus_probs = {k: (round(stats.median(vs),4) if vs else None) for k,vs in probs_acc.items()}
            consensus_overround = round(stats.median(overrounds),4) if overrounds else None
        else:
        ...
        consensus_entry = {
            ...
            'bookmakers_considered': sum(1 for bm in use_bms if (bm.get('h2h') or {}).get('implied_probabilities')),
        ...
        if model_prediction and all(consensus_probs.values()):  # ensure no None
            ...
            edges = {k: round(model_probs[k] - consensus_probs[k],4) for k in ('H','D','A') if consensus_probs[k] is not None}
            ...
            edges_list.append({
                'home_team': home,
                'away_team': away,
                'model_probs': None if not model_prediction else {
...
def consolidated_weeks_report(weeks: List[int]) -> Dict[str, Any]:
    ...
    for wk in weeks:
        ...
        for m, entries in markets.items():
            ...
            acc = [e.get('model_accuracy') for e in entries if isinstance(e, dict) and e.get('model_accuracy') is not None]
            rec = [e.get('recommended_edges') for e in entries if isinstance(e, dict) and e.get('recommended_edges') is not None]
            evaluable = [e.get('evaluable_with_actual_and_probs') for e in entries if isinstance(e, dict) and e.get('evaluable_with_actual_and_probs') is not None]
            summary[m] = {
                'count': len(entries),
...
def main():
    ...
    p_retrain.add_argument('--level', choices=['major','minor','patch'], default='patch')
    sub.add_parser('rebuild', help='Rebuild all predictions (reset cache in-memory)')
    p_sum = sub.add_parser('summary', help='Summarize prediction distribution for a week')
    p_sum.add_argument('--week', type=int, default=1)
    sub.add_parser('mark-trained', help='Force-set model metadata is_trained=True (repair)')
    p_full = sub.add_parser('full', help='Retrain, rebuild, then summarize week 1 (or provided) in one process')
    p_full.add_argument('--level', choices=['major','minor','patch'], default='patch')
    p_full.add_argument('--week', type=int, default=1)
    p_recon = sub.add_parser('reconcile', help='Reconcile completed matches for a week (optionally using manual results)')
    p_recon.add_argument('--week', type=int, default=1)
    p_recon.add_argument('--manual', type=str, default=None, help='Path to manual results JSON file')
    p_recon.add_argument('--force-repredict', action='store_true', help='Force regeneration of predictions even if cached')
    p_recon.add_argument('--apply-goal-adjust', action='store_true', help='Apply historical bias correction offsets to goals before reconciliation')
    p_fetch = sub.add_parser('fetch-scores', help='Fetch final scores from Football-Data.org API and write manual results file')
    ...
    p_fetch.add_argument('--reconcile', action='store_true', help='Immediately run reconciliation after fetch')
    p_fetch.add_argument('--force-repredict', action='store_true', help='Force regeneration during auto-reconcile')
    p_diag = sub.add_parser('diagnostics', help='Extended diagnostics (bias, calibration, adjustments)')
    p_diag.add_argument('--week', type=int, default=None, help='Limit diagnostics to a specific week')
    p_diag.add_argument('--calibration', action='store_true', help='Include probability calibration bins')
    p_diag.add_argument('--goal-adjust', action='store_true', help='Include suggested goal adjustment offsets')
    p_odds = sub.add_parser('ingest-odds', help='Ingest historic odds CSV files (local)')
    p_odds.add_argument('--path', type=str, default='data/odds', help='Directory or single CSV file path containing historic odds')
    p_download_hist = sub.add_parser('download-historic-odds', help='Download EPL historic CSV odds then ingest')
    p_download_hist.add_argument('--start-year', type=int, default=2020, help='Starting season year (e.g. 2020 for 2020-2021)')
    p_download_hist.add_argument('--end-year', type=int, default=2024, help='Ending season year inclusive')
    p_download_hist.add_argument('--overwrite', action='store_true', help='Overwrite existing CSVs')
    p_download_hist.add_argument('--out-dir', type=str, default='data/odds', help='Target directory for downloaded CSVs')
    p_market_rows = sub.add_parser('market-rows', help='Show count and sample of market-derived historic training rows')
    p_market_rows.add_argument('--limit', type=int, default=5, help='Sample size to display')
    p_live_odds = sub.add_parser('live-odds', help='Fetch (best-effort) odds from Football-Data.org for a matchday')
    ...
    p_live_odds.add_argument('--deep', action='store_true', help='Per-match follow-up requests for missing odds')
    p_odds_api = sub.add_parser('odds-api', help='Fetch odds via external Odds API (requires ODDS_API_KEY)')
    p_odds_api.add_argument('--sport', type=str, default='soccer_epl', help='Sport key (e.g., soccer_epl)')
    p_odds_api.add_argument('--regions', type=str, default='eu', help='Regions parameter (api docs)')
    p_odds_api.add_argument('--markets', type=str, default='h2h', help='Markets (default h2h)')
    p_odds_api.add_argument('--edge-threshold', type=float, default=0.05, help='Edge threshold for recommendations')
    # Calibration
    p_cal = sub.add_parser('calibrate', help='Fit temperature scaling up to a given week (uses completed matches only)')
    p_cal.add_argument('--up-to-week', type=int, default=1, help='Include matches up to and including this week')
    p_cmp = sub.add_parser('week-odds-compare', help='Compare model vs historic market consensus vs actual outcomes for a week')
    p_cmp.add_argument('--week', type=int, default=1)
    p_cmp.add_argument('--edge-threshold', type=float, default=0.05)
    p_cmp_tot = sub.add_parser('week-totals-compare', help='Compare model vs market totals (O/U 2.5) vs actual for a week')
    ...
    p_fh = sub.add_parser('week-firsthalf-compare', help='First Half totals compare (O/U line) vs actual for a week')
    ...
    p_sh = sub.add_parser('week-secondhalf-compare', help='Second Half totals compare (O/U line) vs actual for a week')
    ...
    # Corners totals
    p_cor = sub.add_parser('week-corners-compare', help='Corners totals compare (O/U line) vs actual for a week')
    ...
    p_dc = sub.add_parser('week-dc-compare', help='Double Chance compare (1X, 12, X2) vs market')
    ...
    p_dnb = sub.add_parser('week-dnb-compare', help='Draw No Bet compare (home/away) vs market')
    ...
    # Team totals
    p_tt_g = sub.add_parser('week-team-goals-compare', help='Team goals totals compare (home/away) O/U vs actual')
    ...
    p_tt_c = sub.add_parser('week-team-corners-compare', help='Team corners totals compare (home/away) O/U vs actual')
    ...
    p_cm = sub.add_parser('fetch-corners-markets', help='Fetch corners totals markets via Odds API (requires ODDS_API_KEY)')
    ...
    p_cm.add_argument('--from', dest='ts_from', default=None, help='ISO datetime lower bound for historical fetch')
    p_cm.add_argument('--to', dest='ts_to', default=None, help='ISO datetime upper bound for historical fetch')
    ...
    p_bcm = sub.add_parser('bovada-corners-backfill', help='Backfill corners totals and team corners from Bovada into markets file')
    p_bcm.add_argument('--out', default='data/corners_market_lines_manual.json')
    # Consolidated report
    p_cons = sub.add_parser('consolidated-report', help='Consolidate cached week results into one artifact')
    p_cons.add_argument('--weeks', type=str, default='1,2,3', help='Comma-separated week numbers (e.g., 1,2,3)')
    # Corners calibration
    p_cal_cor = sub.add_parser('calibrate-corners', help='Fit corners calibration on historical weeks and persist')
    p_cal_cor.add_argument('--up-to-week', type=int, default=6, help='Use completed weeks up to and including this week for calibration')
    p_cal_cor.add_argument('--line', type=float, default=9.5)
    p_cal_cor.add_argument('--threshold-week', type=int, default=7, help='Blend market odds from this week onward')
    p_cal_cor.add_argument('--blend-weight', type=float, default=0.25, help='Weight for market over prob in blending (0..0.9)')
    # Odds snapshot
    p_snap = sub.add_parser('odds-snapshot', help='Snapshot all week compares to a JSON file under data/odds_snapshots')
    ...
    p_wf = sub.add_parser('walkforward', help='Walk-forward: predict→reconcile→calibrate→retrain for a week range')
    p_wf.add_argument('--start-week', type=int, default=1)
    p_wf.add_argument('--end-week', type=int, default=5)
    p_wf.add_argument('--retrain-level', choices=['major','minor','patch'], default='patch')
    p_wf.add_argument('--no-fetch', action='store_true', help='Do not fetch scores; use existing manual files if present')
    p_wf.add_argument('--apply-goal-adjust', action='store_true', help='Apply historical goal bias adjustment during reconciliation')
    # Daily update
    p_daily = sub.add_parser('daily-update', help='Run daily update: refresh schedule, fetch scores, reconcile, update odds, retrain, rebuild, snapshot')
    p_daily.add_argument('--retrain-level', choices=['major','minor','patch'], default='patch')
    ...
    p_daily.add_argument('--oddsapi-bookmakers', default='pinnacle,bet365,williamhill,unibet,betfair_ex')
    ...
    if args.cmd == 'retrain':
        ...
        result = reconcile_week(args.week, manual_path=args.manual, force_repredict=args.force_repredict, apply_goal_adjust=args.apply_goal_adjust)
        path = _write_output(f'reconcile_w{args.week}', result)
        print(json.dumps({'saved': path, **result}, indent=2))
    elif args.cmd == 'fetch-scores':
        out = fetch_scores(args.competition, args.season, args.week, auto_reconcile=args.reconcile, force_repredict=args.force_repredict)
        path = _write_output(f'fetch_scores_w{args.week}', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'diagnostics':
        out = diagnostics(args.week, calibration=getattr(args,'calibration', False), goal_adjust=getattr(args,'goal_adjust', False))
        ...
        if download_epl_history is None:
            ...
            dl = download_epl_history(args.start_year, args.end_year, out_dir=args.out_dir, overwrite=args.overwrite)
            if 'error' in dl:
                out = dl
            else:
        ...
        try:
            ...
            feature_keys = sorted({k for r in sample for k in r.keys()}) if sample else []
            out = {
                'count': count,
        ...
        out = fetch_week_odds(args.competition, args.season, args.matchday, deep_lookup=args.deep)
        ...
        if fetch_h2h_odds is None:
            ...
            odds_raw = fetch_h2h_odds(sport_key=args.sport, regions=args.regions, markets=args.markets)
            if 'error' in odds_raw:
                out = odds_raw
            else:
                consensus_edges = _compute_consensus_and_edges(odds_raw, edge_threshold=args.edge_threshold)
        ...
        payload['markets']['totals_full'] = compare_week_totals(week, line=2.5, edge_threshold=edge)
        payload['markets']['totals_first_half'] = compare_week_first_half_totals(week, line=1.0, edge_threshold=edge)
        payload['markets']['totals_second_half'] = compare_week_second_half_totals(week, line=1.0, edge_threshold=edge)
        payload['markets']['corners_totals'] = compare_week_corners_totals(week, line=9.5, edge_threshold=edge)
        payload['markets']['team_goals_home'] = compare_week_team_goals_totals(week, side='home', line=1.5, edge_threshold=edge)
        payload['markets']['team_goals_away'] = compare_week_team_goals_totals(week, side='away', line=1.5, edge_threshold=edge)
        payload['markets']['team_corners_home'] = compare_week_team_corners_totals(week, side='home', line=4.5, edge_threshold=edge)
        payload['markets']['team_corners_away'] = compare_week_team_corners_totals(week, side='away', line=4.5, edge_threshold=edge)
        out_dir = Path('data/odds_snapshots'); out_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')
        out_path = out_dir / f"week_{week}_odds_snapshot_{ts}.json"
        out_path.write_text(json.dumps(payload, indent=2), encoding='utf-8')
        print(json.dumps({'saved': str(out_path), 'markets': list(payload['markets'].keys())}, indent=2))
        ...
        out = daily_update(
            ...
            odds_api_bookmakers=getattr(args, 'oddsapi_bookmakers', 'pinnacle,bet365,williamhill,unibet,betfair_ex'),
        )
        path = _write_output('daily_update', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-totals-compare':
        out = compare_week_totals(args.week, line=getattr(args,'line',2.5), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f'week{args.week}_totals_compare', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-firsthalf-compare':
        out = compare_week_first_half_totals(args.week, line=getattr(args,'line',1.0), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f'week{args.week}_firsthalf_compare', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-secondhalf-compare':
        out = compare_week_second_half_totals(args.week, line=getattr(args,'line',1.0), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f'week{args.week}_secondhalf_compare', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-corners-compare':
        out = compare_week_corners_totals(args.week, line=getattr(args,'line',9.5), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f'week{args.week}_corners_compare', out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-dc-compare':
        out = compare_week_double_chance(args.week, market=getattr(args,'market','1X'), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f"week{args.week}_dc_{args.market}_compare", out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-dnb-compare':
        out = compare_week_dnb(args.week, side=getattr(args,'side','home'), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f"week{args.week}_dnb_{args.side}_compare", out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-team-goals-compare':
        out = compare_week_team_goals_totals(args.week, side=getattr(args,'side','home'), line=getattr(args,'line',1.5), edge_threshold=getattr(args,'edge_threshold',0.05))
        path = _write_output(f"week{args.week}_team_goals_{args.side}_compare", out)
        print(json.dumps({'saved': path, **out}, indent=2))
    elif args.cmd == 'week-team-corners-compare':
        out = compare_week_team_corners_totals(args.week, side=getattr(args,'side','home'), line=getattr(args,'line',4.5), edge_threshold=getattr(args,'edge_threshold',0.05))
        ...
        if 'error' in res:
            ...
            with open(out_path, 'w', encoding='utf-8') as f:
                _json.dump({'records': merged, 'source': 'oddsapi', 'historical': res.get('historical')}, f, indent=2)
            totals, team = reload_market_store()
            print(_json.dumps({'saved': out_path, 'added': len(new_recs), 'total_records': len(merged), 'reloaded': {'totals': totals, 'team': team}}, indent=2))
        ...
        for r in rows:
            try:
                ...
                added += 1
        with open(out_path, 'w', encoding='utf-8') as f:
            _json.dump({'records': merged, 'source': 'bovada', 'added_now': added}, f, indent=2)
        totals, team = reload_market_store()
        print(_json.dumps({'saved': out_path, 'added_now': added, 'total_records': len(merged), 'reloaded': {'totals': totals, 'team': team}}, indent=2))
    elif args.cmd == 'consolidated-report':
        weeks = [int(w.strip()) for w in str(getattr(args,'weeks','1,2,3')).split(',') if w.strip().isdigit()]
        ...
        for w in range(1, up_to + 1):
            for m in weeks.get(w, []):
                ...
                zs.append(z); ys.append(y); used += 1
        from ..services.corners_calibration_service import corners_calibration_service
        params = corners_calibration_service.fit(zs, ys, up_to_week=up_to)
        corners_calibration_service.set_market_blend(getattr(args,'threshold_week',7), getattr(args,'blend_weight',0.25))
