        ...
        return [
            {
                ...
                "goals_per_match": round(
                    player.get("goals", 0) / max(player.get("matches_played", 1), 1), 2
                ),
                "shots_per_match": round(
                    player.get("shots", 0) / max(player.get("matches_played", 1), 1), 2
                ),
            }
        ...
        if 1 <= player_id <= len(players):
            ...
            return {
                ...
                "stats": {
                    ...
                    "shot_accuracy": round(
                        (
                            player.get("shots_on_goal", 0)
                            / max(player.get("shots", 1), 1)
                        )
                        * 100,
                        1,
                    )
                    if player.get("shots", 0) > 0
                    else 0,
                },
            }
        else:
...
async def get_top_scorers(limit: int = Query(10, ge=1, le=50)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching top scorers: {str(e)}"
        )
...
async def get_goalkeepers(limit: int = Query(20, ge=1, le=50)):
    ...
    try:
        players = soccer_data.get_player_statistics()
        goalkeepers = [
            player for player in players if player.get("position") == "Goalkeeper"
        ]
        ...
        return [
            {
                ...
                "saves": player.get("saves", 0),
                "matches_played": player.get("matches_played", 0),
                "saves_per_match": round(
                    player.get("saves", 0) / max(player.get("matches_played", 1), 1), 2
                ),
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching goalkeepers: {str(e)}"
        )
