...
def _key(date: Optional[str], home: Optional[str], away: Optional[str]) -> Optional[str]:
...
def _lookup_from_football_data(df: pd.DataFrame, date: str, home: str, away: str) -> Optional[Tuple[int, int, int]]:
    ...
    try:
        ...
        d = pd.to_datetime(df['Date'], errors='coerce', dayfirst=True).dt.strftime('%Y-%m-%d')
    except Exception:
...
def _lookup_match_corners(df: pd.DataFrame, date: str, home: str, away: str) -> Optional[Tuple[int, int, int]]:
    corner_col = _find_corner_column(df)
    if not corner_col:
        return None
    ...
    try:
        ...
        df_local['__date__'] = pd.to_datetime(df_local['date'], dayfirst=True, errors='coerce').dt.strftime('%Y-%m-%d')
    except Exception:
...
def import_weeks(weeks: Iterable[int], out_path: str, use_fbref: bool = False) -> Dict[str, Any]:
        ...
        for m in wk:
            ...
            for csv_path in local_csvs:
                try:
                    ...
                    if all(col in df.columns for col in ['Date', 'HomeTeam', 'AwayTeam', 'HC', 'AC']):
                        trip = _lookup_from_football_data(df, str(date), str(home), str(away))
                        if trip is not None:
                            break
                except Exception:
                    continue
    ...
    with open(out_path, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['date', 'home_team', 'away_team', 'home_corners', 'away_corners', 'total_corners']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for rec in existing.values():
            writer.writerow(rec)

    return {"success": True, "weeks": list(weeks), "considered": considered, "wrote": written, "out": out_path}


def main():
    parser = argparse.ArgumentParser(description='Import corners actuals from FBref via soccerdata')
    parser.add_argument('--weeks', type=str, default='1,2,3', help='Comma-separated game weeks (e.g., 1,2,3)')
    parser.add_argument('--out', type=str, default=os.path.join('data', 'corners_actuals_2025_26.csv'), help='Output CSV path')
    parser.add_argument('--use-fbref', action='store_true', help='Allow FBref network fetch if local CSVs are unavailable')
