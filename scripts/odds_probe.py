...
for lg in leagues:
    ...
    upcoming = [m for m in matches if (m.get('status') or '').lower() in ('scheduled','timed','postponed','')]
    ...
    for m in upcoming[:4]:
        ...
        print('  ', (norm(home) or home), 'vs', (norm(away) or away), '-> provider:', provider, 'H2H:', bool(mw))
