...
def get_json(path: str, *, params: Dict[str, Any] | None = None, timeout: float | None = None):
    url = f"{BASE_URL}{path}"
...
def main() -> int:
    ...
    for lg in [l.strip() for l in LEAGUES if l.strip()]:
        ...
        try:
            ...
            wod = get_json(f"/api/betting/odds/week/{cw}", params={"league": lg}, timeout=20)
            ...
            oc = get_json(f"/api/game-weeks/{cw}/odds-compare", params={"league": lg}, timeout=25)
