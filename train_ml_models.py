...
class AdvancedMLTrainer:
    ...
    def prepare_match_features(self, matches: List[Dict]) -> pd.DataFrame:
        ...
        logger.info(f"✅ Features prepared: {df.shape[0]} matches, {df.shape[1]} features")
        return df
    ...
    def train_goals_predictor(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        feature_columns = [
            ...
            'effective_home_strength', 'gameweek'
        ]
        ...
        results = {
            ...
            'performance': {
                ...
                'total_goals_rmse': np.sqrt(mean_squared_error(y_total_test, total_pred)),
            }
        }
        
        logger.info(f"✅ Goals models trained:")
        logger.info(f"   Home goals RMSE: {results['performance']['home_goals_rmse']:.3f}")
        logger.info(f"   Away goals RMSE: {results['performance']['away_goals_rmse']:.3f}")
        logger.info(f"   Total goals RMSE: {results['performance']['total_goals_rmse']:.3f}")
    ...
    def train_advanced_stats_models(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Advanced stats models trained for {len(stats_models)} statistics")
        
        return results
    
    def save_models(self, goals_results: Dict, result_results: Dict, stats_results: Dict):
        ...
        joblib.dump(result_results, f"{self.models_dir}/result_classifier_{timestamp}.pkl")
    ...
    def train_all_models(self):
        ...
        try:
            ...
            logger.info(f"📊 Training dataset: {df.shape[0]} matches, {df.shape[1]} features")
            ...
            self.generate_training_summary(goals_results, result_results, stats_results, len(historical_matches))
    ...
    def generate_training_summary(self, goals_results: Dict, result_results: Dict, stats_results: Dict, total_matches: int):
